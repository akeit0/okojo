...
- [x] adopt the canonical flat parser/compiler in all production, module, wrapper, REPL, tool, and benchmark entry points
  - Legacy `JsCompiler` partials, compiler context, experimental namespace, and planned/legacy switches are deleted; `JsCompiler` is now the canonical public facade over the flat compiler.
...
- [x] canonical flat compiler coverage baseline: language, built-ins, and intl402 non-annexB sweeps passed in the pre-cutover gates; keep those caches stable while optimizing.
