...
- [ ] direct-flat built-ins remaining (1 deterministic failure, passes under production): AsyncFromSyncIteratorPrototype/next/for-await-next-rejected-promise-close - the async abrupt-close dispatch carries ~2 extra microtask hops vs V8, so f's rejection lands after the harness's tick-2 marker; root cause is in the async driver's promise-resume layering (AttachAsyncAwaitContinuation / PromiseThenResumeAsync), not in emitted code. The 4 Array shift/pop sparse-elements failures were fixed by emitting StaGlobal (assignment semantics) for repeated global `var` redeclarations instead of StaGlobalInit, whose undefined-init no-op path silently dropped later initializers
