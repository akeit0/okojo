...
- [ ] direct-flat built-ins remaining (5 deterministic failures, all pass under production): (a) Array.prototype.shift/pop on sparse generic arrays - second shift returns stale element 0 instead of undefined; planned and production emit equivalent StaKeyedProperty sequences, so the divergence is a runtime elements-kind transition difference triggered by the planned feedback/IC path (minimal repro: x=[]; x[0]=0; x[3]=3; x.shift(); x.length=1; x.shift() returns 0) - 4 tests; (b) AsyncFromSync rejected-promise-close SameValue(2,1) - residual ~2-turn microtask delta in async abrupt-close dispatch - 1 test
