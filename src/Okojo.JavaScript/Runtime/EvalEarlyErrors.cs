...
internal static class EvalEarlyErrors
{
    ...
    )
    {
        switch (statement)
        {
            case JsExpressionStatement expressionStatement:
                return TryFindInvalidNewTarget(
            ...
            case JsIfStatement ifStatement:
                if (
                    TryFindInvalidNewTarget(ifStatement.Test, allowNewTarget, out position)
            ...
            case JsForStatement forStatement:
                if (
                    forStatement.Init is JsExpression initExpression
            ...
            case JsForInOfStatement forInOfStatement:
                if (
                    forInOfStatement.Left is JsExpression leftExpression
            ...
            case JsVariableDeclarationStatement variableDeclaration:
                if (
                    variableDeclaration.BindingInitializer is not null
                    && TryFindInvalidNewTarget(
                        variableDeclaration.BindingInitializer,
                        allowNewTarget,
                        out position
                    )
                )
            ...
            case JsFunctionDeclaration functionDeclaration:
                return TryFindInvalidNewTargetInFunction(
                    functionDeclaration.ParameterInitializers,
            ...
            case JsTryStatement tryStatement:
                if (TryFindInvalidNewTarget(tryStatement.Block, allowNewTarget, out position))
                    return true;
                if (
                    tryStatement.Handler?.BindingPattern is not null
                    && TryFindInvalidNewTarget(
                        tryStatement.Handler.BindingPattern,
                        allowNewTarget,
                        out position
                    )
                )
            ...
            case JsSwitchStatement switchStatement:
                ...
                for (var i = 0; i < switchStatement.Cases.Count; i++)
                {
                    ...
                    if (
                        switchCase.Test is not null
                        && TryFindInvalidNewTarget(switchCase.Test, allowNewTarget, out position)
                    )
                        return true;
                    if (
                        TryFindInvalidNewTarget(switchCase.Consequent, allowNewTarget, out position)
                    )
                        return true;
                }
    ...
    )
    {
        for (var i = 0; i < expressions.Count; i++)
        {
            ...
            if (
                ...
                && TryFindInvalidNewTarget(expression, allowNewTarget, out position)
            )
                return true;
        }
    ...
    )
    {
        switch (expression)
        {
            ...
            case JsAssignmentExpression assignmentExpression:
                if (
                    TryFindInvalidNewTarget(assignmentExpression.Left, allowNewTarget, out position)
                )
                    return true;
                return TryFindInvalidNewTarget(
            ...
            case JsTaggedTemplateExpression taggedTemplateExpression:
                if (
                    ...
                    return true;
                return TryFindInvalidNewTarget(
                    taggedTemplateExpression.Template.Expressions,
                    allowNewTarget,
                    out position
                );
    ...
    {
        if (
            classExpression.ExtendsExpression is not null
            && TryFindInvalidNewTarget(
                classExpression.ExtendsExpression,
                allowNewTarget,
                out position
            )
        )
        ...
        for (var i = 0; i < classExpression.Elements.Count; i++)
        {
            ...
            if (
                ...
                && TryFindInvalidNewTarget(
                    element.StaticBlock.Statements,
                    allowNewTarget,
                    out position
                )
            )
