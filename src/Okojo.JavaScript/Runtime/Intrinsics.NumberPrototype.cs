...
public partial class Intrinsics
{
    private void InstallNumberPrototypeBuiltins()
    {
        ...
        var toFixedFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
        ...
        var toExponentialFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
                ...
                var digitsNumber = fractionDigitsUndefined
                    ? 0d
                    : realm.ToIntegerOrInfinity(args[0]);
                ...
                return NumberPrecisionFormatting.FormatExponential(value, digits);
            },
            "toExponential",
            1
        );
        var toPrecisionFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
                ...
                if (double.IsNegativeInfinity(value))
                    return "-Infinity";
                if (
                    double.IsInfinity(precisionNumber)
                    || precisionNumber < 1
                    || precisionNumber > 100
                )
                    throw new JsRuntimeException(
                ...
                return NumberPrecisionFormatting.FormatPrecision(value, precision);
            },
            "toPrecision",
            1
        );
        ...
        var numberIsFiniteFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var args = info.Arguments;
        ...
        var numberIsNaNFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var args = info.Arguments;
        ...
        var numberIsIntegerFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var args = info.Arguments;
        ...
        var numberIsSafeIntegerFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var args = info.Arguments;
        ...
        [
            ...
            PropertyDefinition.Mutable(
                atomIsSafeInteger,
                JsValue.FromObject(numberIsSafeIntegerFn)
            ),
        ];
    ...
    private static int GetDigitsArgument(
        JsRealm realm,
    ...
    {
        var formatted = NormalizeExponentialString(
