...
public partial class Intrinsics
{
    private JsValue PromiseRace(JsFunction ctor, in JsValue iterable)
    {
        ...
        try
        {
            ...
            while (
                TryGetPromiseCombinatorNextValue(
                    iteratorObj,
                    capability,
                    out var nextValue,
                    out var done
            ...
            )
            {
                ...
                if (
                    !TryInvokePromiseCombinatorThen(
                        useFastPath,
                        ctor,
                        promiseResolve,
                        iteratorObj,
                        capability,
                        nextValue,
                        JsValue.FromObject(capability.Resolve!),
                        JsValue.FromObject(capability.Reject!)
                    )
                )
                    return JsValue.FromObject(capability.Promise);
            }
            ...
            )
            {
                ...
                if (
                    !TryInvokePromiseCombinatorThen(
                        ...
                        capability,
                        nextValue,
                        JsValue.FromObject(resolveElement),
                        JsValue.FromObject(capability.Reject!)
                    )
                )
                    return JsValue.FromObject(capability.Promise);
            ...
            )
            {
                ...
                if (
                    !TryInvokePromiseCombinatorThen(
                        ...
                        nextValue,
                        JsValue.FromObject(capability.Resolve!),
                        JsValue.FromObject(rejectElement)
                    )
                )
                    return JsValue.FromObject(capability.Promise);
            ...
            )
            {
                ...
                if (
                    !TryInvokePromiseCombinatorThen(
                        ...
                        capability,
                        nextValue,
                        JsValue.FromObject(fulfilled),
                        JsValue.FromObject(rejected)
                    )
                )
                    return JsValue.FromObject(capability.Promise);
        ...
        return new(
            ...
            (in info) =>
            {
                ...
                if (state.Owner.Remaining == 0)
                    realm.Intrinsics.ResolvePromiseCapability(
                        state.Owner.Capability,
    ...
    private JsHostFunction CreatePromiseAllSettledElement(
        PromiseAllSettledState owner,
        int index,
        bool isReject
    )
    {
        return new(
            ...
            (in info) =>
            {
                var state = (PromiseAllSettledElementState)
    ...
    private JsValue CompletePromiseAllSettledElement(
        PromiseAllSettledElementState state,
        in JsValue settledValue
    )
    {
        ...
        FreshArrayOperations.DefineElement(
            state.Owner.Values,
            (uint)state.Index,
            JsValue.FromObject(result)
        );
        state.Owner.Remaining--;
        if (state.Owner.Remaining == 0)
            ResolvePromiseCapability(
                state.Owner.Capability,
                JsValue.FromObject(state.Owner.Values)
            );
        return JsValue.Undefined;
    ...
    private JsValue CreatePromiseAnyAggregateError(JsArray errors)
    {
        ...
        return Realm.ConstructWithExplicitNewTarget(
    ...
    )
    {
        if (useFastPath)
        {
            ...
            if (
                nextObj is JsPromiseObject promise
        ...
        {
            var arg0 = nextValue;
            var nextPromise = Realm.InvokeFunction(
                promiseResolve,
    ...
    private sealed class PromiseAllState(
        JsArray values,
        JsPromiseObject.PromiseCapability capability
    )
    {
    ...
    private sealed class PromiseAnyState(
        JsArray errors,
        JsPromiseObject.PromiseCapability capability
    )
    {
    ...
    private sealed class PromiseAllSettledState(
        JsArray values,
        JsPromiseObject.PromiseCapability capability
    )
    {
    ...
    private sealed class PromiseAllSettledElementState(
        PromiseAllSettledState owner,
        int index,
        bool isReject
    )
