...
public partial class Intrinsics
{
    private void InstallArrayConstructorBuiltins()
    {
        ...
        var isArrayFn = new JsHostFunction(
            Realm,
            "isArray",
            1,
            static (in info) =>
            ...
            (in info) =>
            {
                ...
                if (args.Length > 1 && !args[1].IsUndefined)
                {
                    if (
                        ...
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Array.from mapfn must be a function"
                        );
                    mapFn = callback;
                    if (args.Length > 2)
                        thisArg = args[2];
                ...
                {
                    long index = 0;
                    var result = realm.CreateArrayFromTarget(thisValue, true, 0, "Array.from");
                    var iterator = JsRealm.GetIteratorObjectForArrayFrom(
                        realm,
                        items,
                        iteratorMethod
                    );
                    try
                    {
                        while (true)
                        {
                            ...
                            CreateDataPropertyOrThrowForArrayLike(
                                realm,
                                result,
                                index,
                                mappedValue,
                ...
                var resultArrayLike = realm.CreateArrayFromTarget(
                    thisValue,
                    false,
                    length,
                    "Array.from"
        ...
        var ofFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                )
                {
                    ...
                    var created = realm.ConstructWithExplicitNewTarget(
                        ctor,
                        lenArg.AsSpan(),
                        ctor,
                        0
                    ...
                    if (!created.TryGetObject(out target!))
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Array.of constructor must return an object"
                        );
                }
                else
                {
