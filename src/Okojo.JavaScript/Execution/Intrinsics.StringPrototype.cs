...
public partial class Intrinsics
{
    private void InstallStringPrototypeBuiltins()
    {
        ...
        {
            if (
                !searchValue.TryGetObject(out var searchObj)
                || !searchObj.TryGetPropertyAtom(
                    realm,
                    IdSymbolReplace,
                    out var replacerValue,
                    out _
                )
                || replacerValue.IsUndefined
                || replacerValue.IsNull
            )
            ...
            if (!(replacerValue.Obj is JsFunction replacerFn))
                throw new JsRuntimeException(
                    JsErrorKind.TypeError,
                    "Symbol.replace method must be callable"
                );

            Span<JsValue> replaceArgs = [originalValue, replaceValue];
        ...
        static bool IsRegExpForStringMethod(JsRealm realm, in JsValue value)
        {
            ...
            JsValue matchValue;
            if (
                obj.TryGetPropertyAtom(realm, IdSymbolMatch, out matchValue, out _)
        ...
        var atFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    double.IsPositiveInfinity(relativeIndex)
                    || double.IsNegativeInfinity(relativeIndex)
                )
                    return JsValue.Undefined;
        ...
        var charAtFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var indexOfFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var search =
                    args.Length != 0 ? realm.ToJsStringValueSlowPath(args[0]) : "undefined";
                var start = ClampStringPosition(
            ...
            (in info) =>
            {
                ...
                var end =
                    args.Length > 1 && !args[1].IsUndefined
                        ...
                        : length;
                return JsValue.FromString(
                    GetSubstringByCodeUnit(text, (int)start, (int)Math.Max(start, end))
                );
            },
            "slice",
            2
        ...
        var substrFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var start =
                    ...
                    : startNumber < 0 ? Math.Max(len + (int)startNumber, 0)
        ...
        var startsWithFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                return
                    search.Length > text.Length - start
                    || !text.Slice(start, text.Length - start).StartsWith(search)
                    ? JsValue.False
                    : JsValue.True;
            },
            "startsWith",
        ...
        var endsWithFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                return search.Length > end || !text.Slice(0, end).EndsWith(search)
                    ? JsValue.False
                    : JsValue.True;
            },
            "endsWith",
        ...
        var padStartFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                var fillString =
                    args.Length < 2 || args[1].IsUndefined
                        ? " "
                        : realm.ToJsStringValueSlowPath(args[1]);
        ...
        var padEndFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var repeatFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var replaceFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var replacedByMethod = CallReplaceMethodIfPresent(
                    realm,
                    searchValue,
                    thisValue,
                    replaceValue
                ...
                var builder = new PooledCharBuilder(
                    stackalloc char[
                        Math.Min(text.Length + Math.Max(replacementTemplate.Length, 16), 256)
                    ]
                );
                try
                {
        ...
        var replaceAllFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var splitFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    !separatorValue.IsUndefined
                    && !separatorValue.IsNull
                    && separatorValue.TryGetObject(out var separatorHookObj)
                    && separatorHookObj.TryGetPropertyAtom(
                        realm,
                        IdSymbolSplit,
                        out var splitterValue,
                        out _
                    )
                    && !splitterValue.IsUndefined
                    && !splitterValue.IsNull
                )
                {
                    if (
                        !splitterValue.TryGetObject(out var splitterObj)
                        || splitterObj is not JsFunction splitterFn
                    )
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Symbol.split method must be callable"
                        );
                ...
                if (
                    !separatorValue.IsUndefined
                    && (
                        !separatorValue.TryGetObject(out var separatorObj)
                ...
                if (
                    separatorValue.TryGetObject(out var separatorObject)
                ...
                )
                {
                        ...
                        if (
                            JsRegExpRuntime
                                .Exec(
                                    ...
                                    new(
                                        realm,
                                        separatorRegex.Pattern,
                                        separatorRegex.Flags.Contains('g')
                    ...
                    var splitterFlags = separatorRegex.Flags.Contains('g')
                        ? separatorRegex.Flags
                        : separatorRegex.Flags + "g";
                    var splitter = new JsRegExpObject(
                        realm,
                        separatorRegex.Pattern,
                        splitterFlags,
                        true,
                        ...
                        if (matchedLength == 0)
                        {
                            if (matchIndex == splitStart)
                            {
                                if (matchIndex >= text.Length)
                                    break;
                                splitter.SetNamedSlotUnchecked(
                                    RegExpOwnLastIndexSlot,
                                    JsValue.FromInt32(matchIndex + 1)
                                );
                                continue;
                            }
        ...
        var normalizeFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var matchFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    matcherValue.TryGetObject(out var matcherObj)
                    && matcherObj.TryGetPropertyAtom(
                        realm,
                        IdSymbolMatch,
                        out var matchMethodValue,
                        out _
                    )
                    && !matchMethodValue.IsUndefined
                    && !matchMethodValue.IsNull
                )
                {
                    if (
                        !matchMethodValue.TryGetObject(out var matchMethodObj)
                        || matchMethodObj is not JsFunction matchMethod
                    )
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Symbol.match method must be callable"
                        );
                ...
                var created =
                    args.Length == 0 ? realm.CreateRegExpObject("(?:)", string.Empty)
                    : args[0].TryGetObject(out var reObj) && reObj is JsRegExpObject ? args[0]
                    : realm.CreateRegExpObject(
                        args[0].IsUndefined ? "(?:)" : realm.ToJsStringSlowPath(args[0]),
                        string.Empty
                    );
                if (
                    !created.TryGetObject(out var createdObj)
                    || !createdObj.TryGetPropertyAtom(
                        realm,
                        IdSymbolMatch,
                        out var createdMatchValue,
                        out _
                    )
        ...
        var matchAllFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (regexp.TryGetObject(out var regexpObj))
                {
                    ...
                    if (
                        regexpObj.TryGetPropertyAtom(
                            realm,
                            IdSymbolMatchAll,
                            out var matcherValue,
                            out _
                        )
                        && !matcherValue.IsUndefined
                        && !matcherValue.IsNull
                    )
                    {
                        if (
                            !matcherValue.TryGetObject(out var matcherObj)
                            || matcherObj is not JsFunction matcherFn
                        )
                            throw new JsRuntimeException(
                                JsErrorKind.TypeError,
                ...
                var pattern =
                    args.Length == 0 || regexp.IsUndefined
                        ? "(?:)"
                        : realm.ToJsStringSlowPath(regexp);
                var created = realm.CreateRegExpObject(pattern, "g");
                created.TryGetObject(out var createdObj);
                var createdRegex = (JsRegExpObject)createdObj!;
                if (
                    !createdRegex.TryGetPropertyAtom(
                        realm,
                        IdSymbolMatchAll,
                        out var createdMatcherValue,
                        out _
                    )
                ...
                return realm.InvokeFunction(
                    createdMatcherFn,
                    JsValue.FromObject(createdRegex),
                    createdArgs
                );
            },
            "matchAll",
        ...
        var searchFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    ...
                    && regexpObj.TryGetPropertyAtom(
                        realm,
                        IdSymbolSearch,
                        out var searcherValue,
                        out _
                    )
                    && !searcherValue.IsUndefined
                    && !searcherValue.IsNull
                )
                {
                    if (
                        !searcherValue.TryGetObject(out var searcherObj)
                        || searcherObj is not JsFunction searcherFn
                    )
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Symbol.search method must be callable"
                        );
        ...
        var toLowerCaseFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var toLocaleLowerCaseFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var thisValue = info.ThisValue;
                return JsValue.FromString(
                    JsStringLocaleCaseOperations.ToLocaleLowerCase(
                        realm,
        ...
        var toUpperCaseFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var toLocaleUpperCaseFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var thisValue = info.ThisValue;
                return JsValue.FromString(
                    JsStringLocaleCaseOperations.ToLocaleUpperCase(
                        realm,
        ...
        var localeCompareFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    ...
                    && intlObject.TryGetPropertyAtom(
                        realm,
                        realm.Atoms.InternNoCheck("Collator"),
                        out var ctorValue,
                        out _
                    )
                ...
                )
                {
                    ...
                    var collatorValue = realm.ConstructWithExplicitNewTarget(
                        ctorFn,
                        [locales, options],
                    ...
                    if (collatorValue.TryGetObject(out var collatorObject))
                    {
                        if (collatorObject is JsCollatorObject collator)
                            return JsValue.FromInt32(
                                collator.Compare(text.Flatten(), that.Flatten())
                            );

                        if (
                            collatorObject.TryGetPropertyByAtom(IdCompare, out var compareValue)
                            && compareValue.TryGetObject(out var compareObject)
                            && compareObject is JsFunction compareFn
                        )
                        {
                            Span<JsValue> compareArgs =
                            [
                                JsValue.FromString(text),
                                JsValue.FromString(that),
                            ];
                            return realm.InvokeFunction(
                                compareFn,
                                JsValue.FromObject(collatorObject),
                                compareArgs
                            );
                        }
                    }
        ...
        var trimFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var thisValue = info.ThisValue;
                return JsValue.FromString(
                    TrimEcmaWhitespace(ToCoercibleJsString(realm, thisValue, "trim"), true, true)
                );
            },
            "trim",
            0
        ...
        var trimStartFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                return JsValue.FromString(
                    TrimEcmaWhitespace(
                        ToCoercibleJsString(realm, thisValue, "trimStart"),
                        true,
                        false
                    )
                );
        ...
        var trimEndFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                return JsValue.FromString(
                    TrimEcmaWhitespace(
                        ToCoercibleJsString(realm, thisValue, "trimEnd"),
                        false,
                        true
                    )
                );
        ...
        var isWellFormedFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var toWellFormedFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                var thisValue = info.ThisValue;
                return JsValue.FromString(
                    ToWellFormedString(ToCoercibleJsString(realm, thisValue, "toWellFormed"))
                );
            },
            "toWellFormed",
            0
        ...
        var stringIteratorNextFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    ...
                    || obj is not JsStringIteratorObject iterator
                )
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
        ...
        var stringIteratorSelfFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var thisValue = info.ThisValue;
        ...
        [
            ...
            PropertyDefinition.Mutable(
                IdToLocaleLowerCase,
                JsValue.FromObject(toLocaleLowerCaseFn)
            ),
            ...
            PropertyDefinition.Mutable(
                IdToLocaleUpperCase,
                JsValue.FromObject(toLocaleUpperCaseFn)
            ),
            ...
            PropertyDefinition.Mutable(
                atomTable.InternNoCheck("trimLeft"),
                JsValue.FromObject(trimStartFn)
            ),
            PropertyDefinition.Mutable(
                atomTable.InternNoCheck("trimRight"),
                JsValue.FromObject(trimEndFn)
            ),
        ];
        ...
        var rawFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    !templateObj.TryGetPropertyAtom(realm, IdRaw, out var rawValue, out _)
                    || !realm.TryToObject(rawValue, out var rawObj)
                )
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "String.raw template.raw must be an object"
                    );
                ...
                var literalCount =
                    double.IsNaN(lengthNumber) || lengthNumber <= 0d ? 0
                    : lengthNumber >= 9007199254740991d ? 9007199254740991L
                ...
                for (long i = 0; i < literalCount; i++)
                {
                    ...
                    {
                        var atomIndex = realm.Atoms.InternNoCheck(
