...
public partial class Intrinsics
{
    ...
    {
        var constructorGetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var constructorSetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                {
                    if (
                        !thisObj.DefineOwnDataPropertyExact(
                            realm,
                            atomConstructor,
                            value,
        ...
        var toStringTagGetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                return JsValue.FromString("Iterator");
            },
            "get [Symbol.toStringTag]",
            0
        );

        var toStringTagSetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
            ...
            (in info) =>
            {
                ...
                for (var i = 0; i < args.Length; i++)
                {
                    if (!args[i].TryGetObject(out var itemObj))
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
                            "Iterator.concat item must be an object"
                        );

                    if (
                        !itemObj.TryGetPropertyAtom(
                            realm,
                            IdSymbolIterator,
                            out var methodValue,
                            out _
                        ...
                        || methodObj is not JsFunction iteratorMethod
                    )
                        throw new JsRuntimeException(
                            JsErrorKind.TypeError,
            ...
            (in info) =>
            {
                ...
                if (double.IsNaN(numLimit))
                {
                    ...
                    throw new JsRuntimeException(
                        ...
                        "Iterator.prototype.drop limit is invalid"
                    );
                }

                var integerLimit =
                    double.IsInfinity(numLimit) ? numLimit
                    : numLimit == 0d ? 0d
                ...
                var limit = double.IsPositiveInfinity(integerLimit)
                    ? long.MaxValue
                    : (long)integerLimit;
        ...
        var everyFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    args.Length == 0
                    || !args[0].TryGetObject(out var predicateObj)
                ...
                while (true)
                {
                    var step = CallIteratorHelperMethod(
                        realm,
                        nextValue,
                        iteratorObj,
                        "Iterator next must be callable"
                    ...
                    try
                    {
                        predicateResult = realm.InvokeFunction(
                            predicate,
                            JsValue.Undefined,
                            predicateArgs
                        );
        ...
        var flatMapFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    args.Length == 0
                    || !args[0].TryGetObject(out var mapperObj)
                    || mapperObj is not JsFunction mapper
                )
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
        ...
        var forEachFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    args.Length == 0
                    || !args[0].TryGetObject(out var fnObj)
        ...
        var reduceFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    args.Length == 0
                    || !args[0].TryGetObject(out var reducerObj)
                    || reducerObj is not JsFunction reducer
                )
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                ...
                if (args.Length < 2)
                {
                    var firstStep = CallIteratorHelperMethod(
                        realm,
                        nextValue,
                        iteratorObj,
                        "Iterator next must be callable"
        ...
        var takeFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (double.IsNaN(numLimit))
                {
                    ...
                    throw new JsRuntimeException(
                        JsErrorKind.RangeError,
                        "Iterator.prototype.take limit is invalid"
                    );
                }
        ...
        var disposeFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    iteratorObj.TryGetPropertyAtom(realm, IdReturn, out var returnValue, out _)
                    && !returnValue.IsUndefined
                    && !returnValue.IsNull
                )
                    _ = CallIteratorHelperMethod(
                        realm,
                        returnValue,
                        iteratorObj,
        ...
        var asyncDisposeFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                try
                {
                    ...
                    {
                        var result = CallIteratorHelperMethod(
                            realm,
                            returnValue,
                            iteratorObj,
        ...
        var fromFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (args.Length == 0)
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "Iterator.from requires an input"
                    );
                ...
                if (sourceWasPrimitiveString)
                    if (
                        TryGetIteratorMethodForPrimitiveString(
                            realm,
                            source,
                            out var stringMethodValue
                        )
                    )
                    {
                        if (
                            !stringMethodValue.TryGetObject(out var stringMethodObj)
                            || stringMethodObj is not JsFunction stringMethod
                        )
                            throw new JsRuntimeException(
                                JsErrorKind.TypeError,
                                "Iterator.from input is not iterable"
                            );
        ...
        var asyncIteratorSelfFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
        ...
        var toArrayFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        [
            PropertyDefinition.Mutable(
                ...
                JsValue.FromObject(asyncIteratorSelfFn)
            ),
    ...
    internal static bool TryGetIteratorMethodForPrimitiveString(
        JsRealm realm,
        in JsValue source,
    ...
    private static void BestEffortCloseIteratorForIteratorHelper(
        JsRealm realm,
    ...
    internal readonly record struct IteratorConcatItem(
        JsObject Iterable,
