...
public partial class Intrinsics
{
            ...
            (in info) =>
            {
                ...
                if (
                    ...
                    )
                    {
                        ...
                        if (
                            double.IsNaN(requestedMaxByteLength.Value)
                            || requestedMaxByteLength.Value == 0d
                        )
                ...
                if (
                    ...
                    throw new JsRuntimeException(
                        JsErrorKind.RangeError,
                        "Invalid shared array buffer max length"
                    );
                ...
                uint? maxByteLength = requestedMaxByteLength.HasValue
                    ...
                    : null;
                return new JsArrayBufferObject(
                    realm,
                    new JsArrayBufferObject.SharedBufferStorage(byteLength, maxByteLength),
                    prototype
                );
            },
            "SharedArrayBuffer",
    ...
    private void InstallSharedArrayBufferConstructorBuiltins()
    {
        ...
        var growableGetter = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var buffer = ThisSharedArrayBufferValue(info.Realm, info.ThisValue);
