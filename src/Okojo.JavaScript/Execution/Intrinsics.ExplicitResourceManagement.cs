...
public partial class Intrinsics
{
    ...
    {
        var disposableDisposedGetFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
                    info.ThisValue,
                    false,
        ...
        var disposableUseFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
        ...
        var disposableAdoptFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
        ...
        var disposableDeferFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
        ...
        var disposableDisposeFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
        ...
        var disposableMoveFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
                ...
                stack.State = DisposableStackState.Disposed;
                var moved = new JsDisposableStackObject(
                    info.Realm,
                    false,
                    info.Realm.DisposableStackPrototype
                );
        ...
        var asyncDisposedGetFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
        ...
        var asyncUseFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
                ...
                if (value.IsNull || value.IsUndefined)
                {
                    stack.AddResource(
                        new(
                            JsValue.Undefined,
                            DisposableResourceHint.AsyncDispose,
                            JsValue.Undefined
                        )
                    );
                    return value;
        ...
        var asyncAdoptFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
                ...
                stack.ThrowIfDisposed();
                var value = info.GetArgumentOrDefault(0, JsValue.Undefined);
                var onDispose = RequireCallableArgument(
                    info,
                    1,
                    "AsyncDisposableStack.prototype.adopt"
                );
        ...
        var asyncDeferFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
        ...
        var asyncDisposeFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
                try
                {
                    ...
                    return realm.WrapPromiseValueTask(
                        realm.Intrinsics.DisposeResourcesAsync(resources)
                    );
                }
        ...
        var asyncMoveFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var stack = RequireDisposableStack(
                ...
                var moved = new JsDisposableStackObject(
                    info.Realm,
                    true,
                    info.Realm.AsyncDisposableStackPrototype
                );
                moved.ReplaceResources(movedResources);
                return moved;
            },
        ...
        [
            PropertyDefinition.Mutable(
                IdConstructor,
                JsValue.FromObject(DisposableStackConstructor)
            ),
            ...
            PropertyDefinition.Const(
                IdSymbolToStringTag,
                JsValue.FromString("DisposableStack"),
                configurable: true
            ),
        ];
        ...
        [
            PropertyDefinition.Mutable(
                IdConstructor,
                JsValue.FromObject(AsyncDisposableStackConstructor)
            ),
            ...
            PropertyDefinition.Const(
                IdSymbolToStringTag,
                JsValue.FromString("AsyncDisposableStack"),
                configurable: true
            ),
        ];
        AsyncDisposableStackPrototype.DefineNewPropertiesNoCollision(
            Realm,
            asyncDisposableProtoDefs
        );
    }

    internal JsDisposableStackObject CreateCompilerDisposableStack(bool isAsync)
    {
        return new JsDisposableStackObject(
            Realm,
            isAsync,
    ...
    internal void AddCompilerDisposableResource(
        JsDisposableStackObject stack,
        in JsValue value,
    ...
    )
    {
        ...
        if (value.IsNull || value.IsUndefined)
        {
            if (stack.IsAsync && hint == DisposableResourceHint.AsyncDispose)
                stack.AddResource(
                    new(JsValue.Undefined, DisposableResourceHint.AsyncDispose, JsValue.Undefined)
                );
            return;
        }

        var method = GetDisposeMethod(
            Realm,
            value,
            hint == DisposableResourceHint.AsyncDispose,
            out var resolvedHint
        );
        if (method.IsUndefined)
        {
            throw new JsRuntimeException(
    ...
    internal JsValue DisposeCompilerDisposableStack(
        JsDisposableStackObject stack,
        int completionKind,
    ...
    private static JsDisposableStackObject RequireDisposableStack(
        in JsValue value,
        bool isAsync,
        string operation
    )
    {
        if (
            ...
            || obj is not JsDisposableStackObject stack
            || stack.IsAsync != isAsync
        )
        {
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
    ...
    private static JsFunction RequireCallableArgument(in CallInfo info, int index, string operation)
    {
        ...
        if (!value.TryGetObject(out var obj) || obj is not JsFunction fn)
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                $"{operation} requires a callable argument"
            );
        return fn;
    }

    private static JsValue GetDisposeMethod(
        JsRealm realm,
        in JsValue value,
    ...
    )
    {
        ...
        if (!value.TryGetObject(out var obj))
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "Disposable resource must be an object"
            );
    ...
    {
        if (
            !obj.TryGetPropertyAtom(realm, atom, out var method, out _)
            || method.IsUndefined
            || method.IsNull
        )
            return JsValue.Undefined;
    ...
    private void DisposeResources(List<DisposableResource> resources, JsValue? priorThrown = null)
    {
        ...
        for (var i = resources.Count - 1; i >= 0; i--)
        {
            ...
            catch (Exception ex)
            {
                ...
                completion =
                    completion is JsValue suppressed ? CreateSuppressedErrorValue(error, suppressed)
                    : priorThrown is JsValue prior ? CreateSuppressedErrorValue(error, prior)
                    : error;
            }
        }
    ...
    private async ValueTask DisposeResourcesAsync(
        List<DisposableResource> resources,
        JsValue? priorThrown = null
    )
    {
        JsValue? completion = null;
        for (var i = resources.Count - 1; i >= 0; i--)
    ...
    private void DisposeResource(in DisposableResource resource)
    {
        ...
        if (
            !resource.DisposeMethod.TryGetObject(out var methodObj)
            || methodObj is not JsFunction method
        )
    ...
    private static JsHostFunction CreateAdoptDisposeWrapper(
        JsRealm realm,
        JsFunction onDispose,
        JsValue value
    )
    {
        return new(
    ...
    private static bool RequiresSingleAsyncDisposeTick(List<DisposableResource> resources)
    {
        ...
        {
            if (
                resources[i].Hint != DisposableResourceHint.AsyncDispose
                || !resources[i].DisposeMethod.IsUndefined
            )
                return false;
        }
    ...
    )
    {
        if (argCount != 0)
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "CreateDisposableResourceStack requires no arguments"
            );
    ...
    {
        HandleRuntimeAddCompilerDisposableResource(
            realm,
            ref registers,
            argRegStart,
            argCount,
    ...
    {
        HandleRuntimeAddCurrentModuleCompilerDisposableResource(
            realm,
            ref registers,
            argRegStart,
            argCount,
    ...
    {
        HandleRuntimeDisposeCompilerDisposableResourceStack(
            realm,
            ref registers,
            argRegStart,
            argCount,
    ...
    )
    {
        if (argCount != 2)
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "AddDisposableResource requires two arguments"
            );

        var stackValue = Unsafe.Add(ref registers, argRegStart);
        if (
            !stackValue.TryGetObject(out var stackObject)
            || stackObject is not JsDisposableStackObject stack
        )
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "Explicit resource stack must be an object"
            );

        realm.Intrinsics.AddCompilerDisposableResource(
            stack,
    ...
    )
    {
        if (argCount != 3)
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "DisposeDisposableResourceStack requires three arguments"
            );
        ...
        var completionKind = JsRealm.ToInt32SlowPath(
            realm,
            Unsafe.Add(ref registers, argRegStart + 1)
        );
    ...
    )
    {
        if (argCount != 1)
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "AddCurrentModuleDisposableResource requires one argument"
            );
        ...
        realm.Intrinsics.AddCompilerDisposableResource(
            stack,
            Unsafe.Add(ref registers, argRegStart),
            hint
        );
        acc = JsValue.Undefined;
    }
