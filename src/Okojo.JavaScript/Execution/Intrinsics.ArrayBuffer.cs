...
public partial class Intrinsics
{
        ...
        return new(
            ...
            (in info) =>
            {
                ...
                if (
                    ...
                    if (
                        options.TryGetPropertyByAtom(IdMaxByteLength, out var maxValue)
                        && !maxValue.IsUndefined
                    )
                        requestedMaxByteLength = ToRequestedArrayBufferLength(realm, maxValue);

                if (
                    requestedMaxByteLength.HasValue
                    && requestedByteLength > requestedMaxByteLength.Value
                )
                    throw new JsRuntimeException(
    ...
    private void InstallArrayBufferConstructorBuiltins()
    {
        ...
        var detachedGetter = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                var thisValue = info.ThisValue;
                return ThisArrayBufferValue(realm, thisValue).IsDetached
                    ? JsValue.True
                    : JsValue.False;
            },
            "get detached",
        ...
        var maxByteLengthGetter = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var resizableGetter = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
                var thisValue = info.ThisValue;
                return ThisArrayBufferValue(realm, thisValue).IsResizable
                    ? JsValue.True
                    : JsValue.False;
            },
            "get resizable",
        ...
        var isViewFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var args = info.Arguments;
        ...
        var resizeFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var sliceToImmutableFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var transferFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var transferToFixedLengthFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var transferToImmutableFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
    ...
    private static JsFunction GetArrayBufferSpeciesConstructor(
        JsRealm realm,
        JsArrayBufferObject exemplar
    )
    {
    ...
    private static JsArrayBufferObject ConstructArrayBufferSpeciesResult(
        JsRealm realm,
        JsFunction ctor,
        uint byteLength
    )
    {
        Span<JsValue> lenArg = [JsValue.FromInt32((int)byteLength)];
        var createdValue = realm.ConstructWithExplicitNewTarget(ctor, lenArg, ctor, 0);
        if (
            !createdValue.TryGetObject(out var createdObj)
            || createdObj is not JsArrayBufferObject createdBuffer
        )
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
