...
public partial class Intrinsics
{
    private void InstallObjectPrototypeBuiltins()
    {
        ...
        var hasOwnPropertyFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                ...
                if (
                    ...
                    return typedArrayHandled && !typedArrayDescriptor.IsUndefined
                        ? JsValue.True
                        : JsValue.False;
        ...
        var propertyIsEnumerableFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                )
                {
                    ...
                    if (!typedArrayDescriptor.TryGetObject(out var typedArrayDescriptorObj))
                        return JsValue.False;
                    if (
                        !typedArrayDescriptorObj.TryGetPropertyAtom(
                            realm,
                            IdEnumerable,
                ...
                return
                    TryGetOwnPropertyEnumerable(realm, obj, key, out var hasOwn, out var enumerable)
                    && hasOwn
                    && enumerable
                    ? JsValue.True
                    : JsValue.False;
        ...
        var isPrototypeOfFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var toStringFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                if (thisValue.IsNull)
                    return "[object Null]";
                var obj = thisValue.TryGetObject(out var objectValue)
                    ? objectValue
                    : realm.BoxPrimitive(thisValue);
                ...
                if (
                    obj.TryGetPropertyAtom(realm, IdSymbolToStringTag, out var toStringTag, out _)
        ...
        var toLocaleStringFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                if (
                    !GetPropertyValuePreservingReceiver(
                        realm,
                        thisValue,
                        obj,
                        IdToString,
        ...
        static bool GetPropertyValuePreservingReceiver(
            JsRealm realm,
        ...
        static bool TryGetOwnPropertyEnumerable(
            JsRealm realm,
            JsObject target,
            in JsValue key,
            out bool hasOwn,
        ...
        )
        {
            if (target.TryGetOwnPropertyDescriptorViaTrap(realm, key, out var trapDescriptor))
            {
                ...
                hasOwn = true;
                enumerable =
                    descriptorObject.TryGetPropertyAtom(
                        realm,
                        IdEnumerable,
                        out var enumerableValue,
                        out _
            ...
            if (key.IsSymbol)
            {
                ...
                if (
                    ...
                    && symbolGlobal.TryGetOwnGlobalDescriptorAtom(
                        atom,
                        out var globalSymbolDescriptor
                    )
                )
                {
                    hasOwn = true;
            ...
            if (key.IsNumber)
            {
                ...
                if (
                    numericKey == Math.Truncate(numericKey)
                    && numericKey >= 0d
                    && numericKey <= uint.MaxValue
                )
                {
                ...
                if (realm.Atoms.TryGetInterned(numericName, out var numericAtom))
                    if (
                        target.TryGetOwnNamedPropertyDescriptorAtom(
                            realm,
                            numericAtom,
                            out var numericNamedDescriptor
                        )
                    )
                    {
                        hasOwn = true;
        ...
        [
            ...
            PropertyDefinition.Mutable(
                atomPropertyIsEnumerable,
                JsValue.FromObject(propertyIsEnumerableFn)
            ),
