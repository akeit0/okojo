...
public partial class Intrinsics
{
        ...
        return new(
            ...
            (in info) =>
            {
                ...
                if (!info.IsConstruct)
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "WeakRef constructor must be called with new"
                    );
    ...
    private void InstallWeakCollectionBuiltins()
    {
        ...
        var weakMapGetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (args[0].TryGetObject(out var objectKey))
                    return map.TryGetValue(objectKey, out var objectValue)
                        ? objectValue
                        : JsValue.Undefined;
                if (CanBeHeldWeakly(args[0]))
                    return map.TryGetValue(args[0].AsSymbol(), out var symbolValue)
                        ? symbolValue
                        : JsValue.Undefined;
                return JsValue.Undefined;
            },
        ...
        var weakMapSetFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var weakMapHasFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var weakMapDeleteFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var weakSetAddFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (args.Length == 0)
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "WeakSet value must be an object or symbol"
                    );
                if (args[0].TryGetObject(out var objectKey))
                {
        ...
        var weakSetHasFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var weakSetDeleteFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var weakMapGetOrInsertFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (TryGetWeakCollectionKey(key, out var objectKey, out var symbolKey))
                {
                    if (
                        ...
                        && map.TryGetValue(objectKey, out var existingObjectValue)
                    )
                        return existingObjectValue;
                    if (
                        symbolKey is not null
                        && map.TryGetValue(symbolKey, out var existingSymbolValue)
                    )
        ...
        var weakMapGetOrInsertComputedFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                var value = realm.InvokeFunction(
                    callbackFn,
                    JsValue.Undefined,
                    callbackArgs.AsSpan()
                );
        ...
        var weakRefDerefFn = new JsHostFunction(
            Realm,
            static (in info) =>
            {
                var realm = info.Realm;
        ...
        var finalizationRegistryRegisterFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var finalizationRegistryUnregisterFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        [
            ...
            PropertyDefinition.Mutable(
                atomGetOrInsertComputed,
                JsValue.FromObject(weakMapGetOrInsertComputedFn)
            ),
        ...
        [
            PropertyDefinition.Mutable(
                IdConstructor,
                JsValue.FromObject(FinalizationRegistryConstructor)
            ),
            PropertyDefinition.Mutable(
                IdRegister,
                JsValue.FromObject(finalizationRegistryRegisterFn)
            ),
            PropertyDefinition.Mutable(
                IdUnregister,
                JsValue.FromObject(finalizationRegistryUnregisterFn)
            ),
            PropertyDefinition.Const(
                IdSymbolToStringTag,
                JsValue.FromString("FinalizationRegistry"),
                configurable: true
            ),
        ];
        FinalizationRegistryPrototype.DefineNewPropertiesNoCollision(
            Realm,
            finalizationRegistryProtoDefs
        );
    }
    ...
    private static JsFinalizationRegistryObject ThisFinalizationRegistryValue(
        JsRealm realm,
        in JsValue value
    )
    {
    ...
    private static void PopulateWeakMapFromIterable(
        JsRealm realm,
        JsWeakMapObject map,
        JsObject iterable
    )
    {
        const int atomSet = IdSet;
        if (
            !map.TryGetPropertyAtom(realm, atomSet, out var adderValue, out _)
        ...
        while (true)
        {
            ...
            try
            {
                entryValue = StepWeakCollectionIterator(
                    realm,
                    iteratorObj,
                    nextFn,
                    out var done,
            ...
            try
            {
                if (!realm.TryToObject(entryValue, out var entryObj))
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "WeakMap iterable entry is not an object"
                    );
                _ = entryObj.TryGetElement(0, out var keyValue);
                _ = entryObj.TryGetElement(1, out var mappedValue);
                var args = new InlineJsValueArray2 { Item0 = keyValue, Item1 = mappedValue };
    ...
    private static void PopulateWeakSetFromIterable(
        JsRealm realm,
        JsWeakSetObject set,
        JsObject iterable
    )
    {
        ...
        while (true)
        {
            ...
            try
            {
                value = StepWeakCollectionIterator(
                    realm,
                    iteratorObj,
                    nextFn,
                    out var done,
    ...
    private static bool TryGetWeakCollectionKey(
        in JsValue key,
        out JsObject? objectKey,
        out Symbol? symbolKey
    )
    {
    ...
    private static JsObject GetWeakCollectionIterator(
        JsRealm realm,
        JsObject iterable,
    ...
    )
    {
        if (
            ...
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                $"{collectionName} iterable is not iterable"
            );
        ...
        if (!iteratorValue.TryGetObject(out var iteratorObj))
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                $"{collectionName} iterator is not an object"
            );
        return iteratorObj;
    }

    private static JsFunction GetWeakCollectionIteratorNext(
        JsRealm realm,
    ...
    )
    {
        ...
        if (
            !iteratorObj.TryGetPropertyAtom(realm, atomNext, out var nextMethod, out _)
            || !nextMethod.TryGetObject(out var nextObj)
            || nextObj is not JsFunction nextFn
        )
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                $"{collectionName} iterator next is not callable"
            );

        return nextFn;
    }

    private static JsValue StepWeakCollectionIterator(
        JsRealm realm,
        JsObject iteratorObj,
        JsFunction nextFn,
        out bool done,
    ...
    )
    {
        var step = realm.InvokeFunction(
            nextFn,
            JsValue.FromObject(iteratorObj),
            ReadOnlySpan<JsValue>.Empty
        );
    ...
    private static void BestEffortIteratorCloseForWeakCollection(
        JsRealm realm,
        JsObject iteratorObj
    )
    {
        try
        {
            ...
            if (
                !iteratorObj.TryGetPropertyAtom(realm, atomReturn, out var returnMethod, out _)
