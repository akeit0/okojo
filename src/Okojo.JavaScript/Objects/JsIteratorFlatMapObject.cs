...
internal sealed class JsIteratorFlatMapObject : JsObject
{
    ...
    internal JsIteratorFlatMapObject(
        JsRealm realm,
        JsObject outerIterator,
        JsValue outerNextMethod,
        JsFunction mapper
    )
        : base(realm)
    {
        ...
        var nextFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                if (
                    ...
                    || thisObj is not JsIteratorFlatMapObject flatMap
                )
    ...
    internal JsValue Next()
    {
        ...
        try
        {
            while (true)
            {
                ...
                {
                    var innerStep = Intrinsics.CallIteratorHelperMethod(
                        Realm,
                        innerNextMethod,
                        innerIterator,
                ...
                var outerStep = Intrinsics.CallIteratorHelperMethod(
                    Realm,
                    outerNextMethod,
                    outerIterator,
                    "Iterator next must be callable"
    ...
    internal JsValue Return()
    {
        ...
        try
        {
            closed = true;
            if (
                innerIterator is not null
                && innerIterator.TryGetPropertyAtom(
                    Realm,
                    IdReturn,
                    out var innerReturnValue,
                    out _
                )
                && !innerReturnValue.IsUndefined
                && !innerReturnValue.IsNull
            )
            ...
            if (
                outerIterator.TryGetPropertyAtom(Realm, IdReturn, out var outerReturnValue, out _)
                && !outerReturnValue.IsUndefined
                && !outerReturnValue.IsNull
            )
                _ = Intrinsics.CallIteratorHelperMethod(
                    Realm,
                    outerReturnValue,
    ...
    private void InitializeInnerIterator(JsValue mapped)
    {
        if (!mapped.TryGetObject(out var mappedObj))
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "Iterator.flatMap mapper result must be object"
            );

        if (
            mappedObj.TryGetPropertyAtom(
                Realm,
                IdSymbolIterator,
