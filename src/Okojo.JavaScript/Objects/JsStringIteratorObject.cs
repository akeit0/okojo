...
internal sealed class JsStringIteratorObject : JsObject
{
    ...
    internal JsStringIteratorObject(JsRealm realm, JsString text)
        : base(realm)
    {
        this.text = text;
    ...
    internal JsValue Next()
    {
        ...
        )
        {
            ...
            return JsValue.FromObject(
                Realm.CreateIteratorResultObject(
                    JsValue.FromString(
                        string.Create(
                            2,
                            (first, second),
        ...
        index++;
        return JsValue.FromObject(
            Realm.CreateIteratorResultObject(JsValue.FromString(first.ToString()), false)
        );
    }
}
