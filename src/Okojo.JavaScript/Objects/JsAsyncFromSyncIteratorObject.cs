...
internal sealed class JsAsyncFromSyncIteratorObject : JsObject
{
    ...
    internal JsAsyncFromSyncIteratorObject(JsRealm realm, JsObject iterator)
        : base(realm)
    {
        ...
        var nextFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                if (
                    ...
                    || thisObj is not JsAsyncFromSyncIteratorObject wrapped
                )
                ...
                return args.Length != 0
                    ? wrapped.Next(args[0], true)
                    : wrapped.Next(JsValue.Undefined, false);
            },
            "next",
            1
        );
        ...
        var returnFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                var args = info.Arguments;
                return args.Length != 0
                    ? wrapped.Return(args[0], true)
                    : wrapped.Return(JsValue.Undefined, false);
            },
            "return",
            1
        );
        ...
        var throwFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                var args = info.Arguments;
                return args.Length != 0
                    ? wrapped.Throw(args[0], true)
                    : wrapped.Throw(JsValue.Undefined, false);
            },
            "throw",
            1
        );
    ...
    private JsValue Next(in JsValue value, bool hasValue)
    {
        try
        {
            ...
            return Realm.CreateAsyncFromSyncIteratorResultPromise(
                resultValue,
                done,
                iterator,
                true
    ...
    private JsValue Throw(in JsValue value, bool hasValue)
    {
        ...
        {
            if (
                !iterator.TryGetPropertyAtom(Realm, IdThrow, out var throwValue, out _)
                || throwValue.IsUndefined
                || throwValue.IsNull
            )
            {
