...
public sealed partial class JsCompiler
{
    private void MarkForLoopHeadBindingsCapturedByNestedFunctionFallback(JsForStatement statement)
    {
        if (!statement.BodyMayCreateNestedFunction)
            return;
        if (
            statement.Init is not JsVariableDeclarationStatement initDeclaration
        ...
        foreach (var declarator in initDeclaration.Declarators)
            if (
                TryResolveLocalBinding(
                    new CompilerIdentifierName(declarator.Name, declarator.NameId),
                    out var resolved
                )
            )
            {
    ...
    private void MarkForInOfHeadBindingsCapturedByNestedFunctionFallback(
    ...
    )
    {
        if (!statement.BodyMayCreateNestedFunction)
            return;
        if (
            statement.Left is not JsVariableDeclarationStatement leftDeclaration
        ...
        {
            if (
                !TryResolveForInOfPerIterationBindingSymbolId(
                    statement,
                    boundIdentifier,
                    out var symbolId
                )
