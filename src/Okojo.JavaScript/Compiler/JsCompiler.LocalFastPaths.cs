...
{
    private bool TryEmitSelfBinaryAssignmentFastPath(
        string resolvedLeftName,
        string sourceLeftName,
        JsExpression rhs
    )
    {
        if (rhs is not JsBinaryExpression bin)
            return false;
        if (bin.Left is not JsIdentifierExpression leftRef)
            return false;
        if (
            !TryGetResolvedAliasSymbolId(
                CompilerIdentifierName.From(leftRef),
                out var leftRefSymbolId,
                out var resolvedLeftRefName
            )
        )
            return false;
        if (!string.Equals(resolvedLeftRefName, resolvedLeftName, StringComparison.Ordinal))
            return false;
        if (
            !TryGetResolvedAliasSymbolId(
                new CompilerIdentifierName(resolvedLeftName),
                out var lhsSymbolId,
                out _
            )
        )
            return false;
        if (!TryGetFastPathLocalRegister(lhsSymbolId, out var lhsReg, out var needsTdzReadCheck))
            return false;
        ...
        )
        {
            ...
            switch (bin.Operator)
            {
                case JsBinaryOperator.Add:
                    EmitImmediateSlotOp(JsOpCode.AddSmi, rhsSmi);
                    break;
                case JsBinaryOperator.Subtract:
                    EmitImmediateSlotOp(JsOpCode.SubSmi, rhsSmi);
                    break;
                case JsBinaryOperator.Multiply:
                    EmitImmediateSlotOp(JsOpCode.MulSmi, rhsSmi);
                    break;
                case JsBinaryOperator.Modulo:
                    EmitImmediateSlotOp(JsOpCode.ModSmi, rhsSmi);
                    break;
                case JsBinaryOperator.Exponentiate:
                    EmitImmediateSlotOp(JsOpCode.ExpSmi, rhsSmi);
                    break;
                case JsBinaryOperator.LessThan:
                    EmitImmediateSlotOp(JsOpCode.TestLessThanSmi, rhsSmi);
                    break;
                case JsBinaryOperator.GreaterThan:
                    EmitImmediateSlotOp(JsOpCode.TestGreaterThanSmi, rhsSmi);
                    break;
                case JsBinaryOperator.LessThanOrEqual:
                    EmitImmediateSlotOp(JsOpCode.TestLessThanOrEqualSmi, rhsSmi);
                    break;
                case JsBinaryOperator.GreaterThanOrEqual:
                    EmitImmediateSlotOp(JsOpCode.TestGreaterThanOrEqualSmi, rhsSmi);
                    break;
                default:
                    return false;
            }
    ...
    private bool TryEmitLiteralInitializerDirectToLocal(
    ...
    )
    {
        ...
        if (
            !TryGetResolvedAliasSymbolId(
                new CompilerIdentifierName(resolvedLocalName),
                out var targetSymbolId,
                out _
    ...
    )
    {
        ...
        if (
            !TryGetResolvedAliasSymbolId(
                new CompilerIdentifierName(resolvedTargetName),
                out var targetSymbolId,
                out _
            )
            || !TryGetFastPathLocalRegister(
                targetSymbolId,
                out var targetReg,
                out var targetNeedsTdzReadCheck
            )
        )
            return false;
        if (
            !TryGetResolvedAliasSymbolId(
                CompilerIdentifierName.From(id),
                out var sourceSymbolId,
                out _
            )
            || !TryGetFastPathLocalRegister(
                sourceSymbolId,
                out var sourceReg,
                out var sourceNeedsTdzReadCheck
            )
        )
            return false;
    ...
    {
        value = 0;
        if (expr is not JsLiteralExpression { Value: double d })
            return false;
        if (IsNegativeZero(d))
            return false;
        if (d % 1 != 0)
    ...
    {
        return op
            is JsBinaryOperator.Add
                or JsBinaryOperator.Subtract
                or JsBinaryOperator.Multiply
                or JsBinaryOperator.Modulo
                or JsBinaryOperator.Exponentiate
                or JsBinaryOperator.LessThan
                or JsBinaryOperator.GreaterThan
                or JsBinaryOperator.LessThanOrEqual
                or JsBinaryOperator.GreaterThanOrEqual;
    }

    private bool TryGetPlainLocalReadRegister(JsExpression expr, out int reg)
    {
        reg = -1;
        if (expr is not JsIdentifierExpression id)
            return false;
        ...
        if (
            ...
            || !TryGetFastPathLocalRegister(
                ...
                out reg,
                out var needsLexicalTdzReadCheck
            )
        )
            return false;
    ...
    private bool TryGetContiguousPlainLocalArgumentRegisters(
