...
public sealed partial class JsCompiler
{
    ...
    )
    {
        ...
        try
        {
            ...
            var privateFieldBindingBySourceName = new Dictionary<string, PrivateFieldBinding>(
            ...
            var privateAccessorInitBySourceName = new Dictionary<string, PrivateAccessorInitPlan>(
                StringComparer.Ordinal
            );
            var privateMethodInitializers = new List<PrivateMethodInitPlan>();
            var publicFieldInitializers = new List<PublicFieldInitPlan>();
            var staticPrivateAccessorInitBySourceName = new Dictionary<
                string,
                PrivateAccessorInitPlan
            >(StringComparer.Ordinal);
            ...
            try
            {
                ...
                foreach (var element in classExpr.Elements)
                {
                    ...
                    if (
                        element.IsPrivate
                        && (element.IsComputedKey || element.ComputedKey is not null)
                    )
                        throw new NotSupportedException(
                    ...
                    if (element.IsPrivate)
                    {
                        ...
                        switch (element.Kind)
                        {
                            ...
                            {
                                if (
                                    privateFieldBindingBySourceName.ContainsKey(sourcePrivateName)
                                    || privateAccessorInitBySourceName.ContainsKey(
                                        sourcePrivateName
                                    )
                                )
                                    throw new NotSupportedException(
                                ...
                                var binding = new PrivateFieldBinding(
                                ...
                                if (!element.IsStatic)
                                {
                                    var initPlan = new PrivateFieldInitPlan(
                                        sourcePrivateName,
                                        binding,
                                        element.FieldInitializer
                                    );
                                    privateFieldInitializers.Add(initPlan);
                                    instanceFieldInitializers.Add(
                                        new(
                            ...
                            {
                                if (
                                    privateFieldBindingBySourceName.TryGetValue(
                                        sourcePrivateName,
                                ...
                                )
                                {
                                    ...
                                    privateFieldBindingBySourceName[sourcePrivateName] =
                                        accessorBinding;
                                    if (element.IsStatic)
                                        staticPrivateAccessorInitBySourceName[sourcePrivateName] =
                                            new(sourcePrivateName, accessorBinding, null, null);
                                    else
                                ...
                                if (!accessorMap.TryGetValue(sourcePrivateName, out var initPlan))
                                    initPlan = new(sourcePrivateName, accessorBinding, null, null);

                                if (element.Value is null)
                                    throw new InvalidOperationException(
                                        "Private accessor function is missing."
                                    );
                            ...
                            case JsClassElementKind.Method:
                            {
                                if (
                                    ...
                                    || staticPrivateAccessorInitBySourceName.ContainsKey(
                                        sourcePrivateName
                                    )
                                )
                                    throw new NotSupportedException(
                                ...
                                if (element.Value is null)
                                    throw new InvalidOperationException(
                                        "Private method function is missing."
                                    );
                                ...
                                {
                                    privateMethodInitializers.Add(
                                        new(sourcePrivateName, methodBinding, element.Value)
                                    );
                                }
                    ...
                    if (element.Kind == JsClassElementKind.Field && !element.IsStatic)
                    {
                        ...
                        instanceFieldInitializers.Add(
                            new(InstanceFieldInitializerKind.PublicField, default, initPlan)
                        );
                    }
                }

                var privateAccessorInitializers = privateAccessorInitBySourceName.Values.ToList();
                var visiblePrivateBindings = MergeVisiblePrivateBindings(
                    privateFieldBindingBySourceName
                );
                activeClassPrivateBindingScopes.Push(visiblePrivateBindings);
                try
                {
                    var inheritedPrivateBrandIds = CollectInheritedPrivateBrandIds(
                        visiblePrivateBindings,
                        instancePrivateBrandId,
                        staticPrivateBrandId
                    );
                    var inheritedActivePrivateBrandMappings =
                        CollectInheritedPrivateBrandMappingsFromActiveScopes(
                            instancePrivateBrandId,
                            staticPrivateBrandId
                        );
                    ...
                    var instanceFieldInitializersUseSuper = instanceFieldInitializers.Any(
                        static initializer =>
                            initializer.Kind == InstanceFieldInitializerKind.PrivateField
                                ? initializer.PrivateField.Initializer is not null
                                    && FieldInitializerUsesDirectSuper(
                                        initializer.PrivateField.Initializer
                                    )
                                : initializer.PublicField.Element.FieldInitializer is not null
                                    && FieldInitializerUsesDirectSuper(
                                        initializer.PublicField.Element.FieldInitializer
                                    )
                    );

                    var ctorParameterPlan = FunctionParameterPlan.FromFunction(constructorFunction);
                    var constructorUsesSuper = FunctionUsesSuper(
                        ctorParameterPlan.Initializers,
                        constructorFunction.Body
                    );
                    var constructorNeedsMethodEnvironment =
                        constructorUsesSuper || instanceFieldInitializersUseSuper;
                    ...
                    {
                        EmitLdaRegister(ctorReg);
                        StoreIdentifier(classLexicalInternalName, true, classLexicalName);
                    }
                    ...
                    var primaryCtorPrivateBrandSourceReg =
                        hasInstancePrivateMembers ? protoReg
                        : hasStaticPrivateMembers ? ctorReg
                        : -1;
                    var instanceMemberPrivateBrandSourceReg =
                        hasInstancePrivateMembers ? protoReg
                        : hasStaticPrivateMembers ? ctorReg
                        : -1;
                    var staticMemberPrivateBrandSourceReg =
                        hasStaticPrivateMembers ? ctorReg
                        : hasInstancePrivateMembers ? protoReg
                        : -1;
                    ...
                    var instanceMemberInheritedPrivateBrandIds = CombinePrivateBrandIds(
                        inheritedPrivateBrandIds,
                        staticPrivateBrandId
                    );
                    var staticMemberInheritedPrivateBrandIds = CombinePrivateBrandIds(
                        inheritedPrivateBrandIds,
                        instancePrivateBrandId
                    );
                    ...
                    activeClassPrivateSourceScopes.Push(
                        new(instancePrivateBrandId, protoReg, staticPrivateBrandId, ctorReg)
                    );
                    try
                    {
                        ...
                        {
                            EmitLdaRegister(ctorReg);
                            EmitSetFunctionPrivateBrandTokenFromAccumulator(
                                primaryCtorPrivateBrandSourceReg
                            );
                            ...
                            for (var i = 0; i < privateMethodInitializers.Count; i++)
                            {
                                ...
                                var methodParameterPlan = FunctionParameterPlan.FromFunction(
                                    initPlan.Function
                                );
                                ...
                                EmitSetFunctionPrivateMethodValueFromAccumulator(
                                    ctorReg,
                        ...
                        var staticPrivateAccessorMaskByKey = new Dictionary<string, byte>(
                            StringComparer.Ordinal
                        );
                        foreach (var element in classExpr.Elements)
                        {
                            ...
                            if (
                                element.Kind
                                is not (JsClassElementKind.Getter or JsClassElementKind.Setter)
                            )
                                continue;
                            var bit = element.Kind == JsClassElementKind.Getter ? (byte)1 : (byte)2;
                            staticPrivateAccessorMaskByKey.TryGetValue(
                                element.Key!,
                                out var existing
                            );
                        ...
                        var staticPrivateAccessorStateByKey = new Dictionary<
                            string,
                            PendingClassAccessorState
                        >(StringComparer.Ordinal);
                        var accessorMaskByKey = new Dictionary<string, byte>(
                            StringComparer.Ordinal
                        );
                        ...
                        foreach (var element in classExpr.Elements)
                        {
                            ...
                            if (element.Kind == JsClassElementKind.Field && !element.IsStatic)
                            {
                                ...
                                computedKeyPlans.Add(
                                    new(InstanceFieldKeyIndex: nextInstanceComputedFieldKeyIndex)
                                );
                        ...
                        foreach (var element in classExpr.Elements)
                        {
                            ...
                            if (!element.IsStatic && element.Kind == JsClassElementKind.Field)
                            {
                                ...
                                if (initPlan.ComputedKeyIndex >= 0)
                                    Debug.Assert(
                                        hasComputedKeyPlan
                                            && computedKeyPlan.InstanceFieldKeyIndex
                                                == initPlan.ComputedKeyIndex
                                    );
                            ...
                            if (
                                ...
                                is not (
                                    ...
                                    or JsClassElementKind.Getter
                                    or JsClassElementKind.Setter
                                    or JsClassElementKind.Field
                            ...
                            if (
                                ...
                                throw new InvalidOperationException(
                                    $"Class {element.Kind} value is missing."
                                );

                            if (
                                !element.IsComputedKey
                                && element.Kind
                                    is JsClassElementKind.Getter
                                        or JsClassElementKind.Setter
                            )
                            {
                                var bit =
                                    element.Kind == JsClassElementKind.Getter ? (byte)1 : (byte)2;
                                var accessorMaskKey = MakeClassAccessorMaskKey(
                                    element.Key!,
                                    element.IsStatic
                                );
                        ...
                        var pairedAccessorStateByKey = new Dictionary<
                            string,
                            PendingClassAccessorState
                        >(StringComparer.Ordinal);
                        foreach (var element in classExpr.Elements)
                        {
                            ...
                            if (element.IsPrivate)
                            {
                                if (
                                    ...
                                    throw new InvalidOperationException(
                                        $"Missing private binding for '{key}'."
                                    );
                                ...
                                switch (element.Kind)
                                {
                                    case JsClassElementKind.Field:
                                    {
                                        if (element.IsStatic)
                                            ...
                                            EmitPrivateFieldInitializerOnTarget(
                                                ctorReg,
                                                privateBinding,
                                                element.FieldInitializer,
                                                key
                                    ...
                                    case JsClassElementKind.Setter:
                                    {
                                        ...
                                        if (!keyHasBothAccessorKinds)
                                        {
                                            EmitPrivateAccessorInitializerOnTarget(
                                                ...
                                                element.Kind == JsClassElementKind.Getter
                                                    ? accessorFn
                                                    : null,
                                                element.Kind == JsClassElementKind.Setter
                                                    ? accessorFn
                                                    : null,
                                                staticMemberInheritedPrivateBrandIds,
                                                ctorReg,
                                        ...
                                        if (
                                            !staticPrivateAccessorStateByKey.TryGetValue(
                                                key,
                                                out var state
                                            )
                                        )
                                    ...
                                    case JsClassElementKind.Method:
                                    {
                                        EmitPrivateMethodInitializerOnTarget(
                                            ctorReg,
                                            key,
                                            privateBinding,
                                            element.Value!,
                            ...
                            switch (element.Kind)
                            {
                                ...
                                {
                                    var elementFunction = element.Value!;
                                    var methodObj = CompileClassElementFunction(
                                        key,
                                        elementFunction,
                                        visiblePrivateBindings,
                                        classLexicalIdentifier
                                    );
                                    EmitDefineClassMethod(
                                        targetReg,
                                        ctorReg,
                                        element,
                                        methodObj,
                                ...
                                case JsClassElementKind.Setter:
                                {
                                    ...
                                    var keyHasBothAccessorKinds =
                                        accessorMaskByKey.TryGetValue(
                                            accessorMaskKey,
                                            out var accessorMask
                                        )
                                        && accessorMask == 3;
                                    if (!keyHasBothAccessorKinds)
                                    {
                                    ...
                                    if (
                                        !pairedAccessorStateByKey.TryGetValue(
                                            accessorMaskKey,
                                            out var state
                                        )
                                    )
    ...
    )
    {
        ...
        try
        {
            ...
            var functionObj = CompileFunctionObject(
                ...
                CreateFunctionShape(false, false, false, true, true),
    ...
    {
        var initializerBody = new JsBlockStatement(
            new JsStatement[]
            {
    ...
    private static bool FieldInitializerUsesDirectSuper(JsExpression initializer)
    {
        return initializer switch
        {
            ...
            JsBinaryExpression b => FieldInitializerUsesDirectSuper(b.Left)
                || FieldInitializerUsesDirectSuper(b.Right),
            ...
            JsArrayExpression a => a.Elements.Any(e =>
                e is not null && FieldInitializerUsesDirectSuper(e)
            ),
                ...
                && (
                    (
                        f.ParameterInitializers?.Any(i =>
                            i is not null && ExpressionContainsSuperInCurrentFunction(i)
                        )
                        ?? false
            ...
            JsYieldExpression y => y.Argument is not null
                && FieldInitializerUsesDirectSuper(y.Argument),
            JsAwaitExpression a => FieldInitializerUsesDirectSuper(a.Argument),
            _ => false,
        };
    }

    private JsBytecodeFunction CompileClassElementFunction(
        string functionName,
    ...
    private void EmitDefineClassMethod(
        int targetReg,
        int classLexicalReg,
        JsClassElement element,
        JsBytecodeFunction methodObj,
        int computedKeyReg,
        int privateBrandSourceReg,
        IReadOnlyList<int>? inheritedPrivateBrandIds,
        int inheritedPrivateBrandSourceReg,
    ...
    private void EmitDefineClassAccessor(
        int targetReg,
        int classLexicalReg,
        JsClassElement element,
    ...
    private void EmitDefineClassField(
        int targetReg,
        JsClassElement element,
        int valueReg,
        int computedKeyReg = -1
    )
    {
        var tempScope = BeginTemporaryRegisterScope();
        try
    ...
    private void EmitClassElementKeyToRegister(
        JsClassElement element,
        int keyReg,
        int computedKeyReg = -1
    )
