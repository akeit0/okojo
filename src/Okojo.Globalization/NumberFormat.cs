...
public sealed class NumberFormat
{
    ...
    public bool TryFormatExactString(string raw, out string formatted)
    {
        ...
        formatted = JoinParts(tupleParts);
        return true;
    }
    ...
    private static List<IntlPart> ToIntlParts(List<(string Type, string Value)> parts)
    {
        var result = new List<IntlPart>(parts.Count);
        foreach (var (type, value) in parts)
            result.Add(new IntlPart(type, value));
        return result;
    }
