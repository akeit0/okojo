...
public static class CalendarData
{
    ...
    public static string? FormatEra(string calendar, int isoYear, string eraWidth)
    {
        ...
        if (thresholdEraRules!.TryGetValue(calendar, out var thresholdRule))
            return FormatEraText(
                isoYear <= thresholdRule.BeforeEndYear ? thresholdRule.Before : thresholdRule.After,
                eraWidth
            );
    ...
    private static void EnsureLoaded()
    {
        ...
        lock (Gate)
        {
            ...
            var thresholdEraRulesBuilder = new Dictionary<string, ThresholdEraRule>(
            ...
            var resourceName =
                assembly
                    ...
                    .FirstOrDefault(static n =>
                        n.EndsWith("CalendarData.txt", StringComparison.Ordinal)
                    )
            ...
            using var stream =
                ...
                ?? throw new InvalidOperationException(
                    "Intl calendar data resource stream not found."
                );
            ...
            eraSuppressedCalendars = eraSuppressedCalendarsBuilder.ToFrozenSet(
                StringComparer.Ordinal
            );
            ...
            supportedCalendarsSorted =
            [
                .. supportedCalendars.OrderBy(static value => value, StringComparer.Ordinal),
            ];
            japaneseEraRules =
            [
                .. japaneseEraList.OrderByDescending(static value => value.StartYear),
            ];
            loaded = true;
        }
    }

    private static void ParseThresholdEraRule(
        string line,
        Dictionary<string, ThresholdEraRule> builder
    )
    {
    ...
    private static JapaneseEraRule ParseJapaneseEraRule(string line)
    {
        ...
        return new(int.Parse(parts[0], CultureInfo.InvariantCulture), new(parts[1], parts[2]));
    }

    private readonly record struct EraText(string ShortText, string LongText);

    private readonly record struct ThresholdEraRule(
        int BeforeEndYear,
        EraText Before,
        EraText After
    );
