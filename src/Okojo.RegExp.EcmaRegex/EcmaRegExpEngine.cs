...
/// <summary>
///     <see cref="IRegExpEngine"/> adapter backed by the EcmaRegex library, giving
///     ECMAScript-conformant parsing and matching (including the <c>v</c> flag,
///     duplicate named groups, lookbehind, and Unicode property escapes).
/// </summary>
public sealed class EcmaRegExpEngine : IRegExpEngine
{
    private readonly EcmaRegexOptions _options;

    public static EcmaRegExpEngine Default { get; } = new();

    public EcmaRegExpEngine(EcmaRegexOptions? options = null)
    {
    ...
    {
        var ecma = EcmaRegExp.Compile(pattern, flags, _options);
        return new RegExpCompiledPattern(
            pattern,
            ecma.FlagsText,
            pattern,
    ...
    {
        if (compiled.EngineState is not EcmaRegExp ecma)
            throw new ArgumentException("Compiled pattern was not created by EcmaRegExpEngine.", nameof(compiled));
    ...
    private static string[] ToNamedGroupNames(EcmaRegExp ecma)
    {
