...
internal static class GlobalExportCollector
{
    public static GlobalTypeModel? Collect(INamedTypeSymbol symbol)
    {
        ...
        var installerMethodName =
            JsExportAttributeHelper.GetNamedString(typeAttribute, "InstallerMethodName")
            ?? "InstallGeneratedGlobals";
        ...
        foreach (var member in symbol.GetMembers())
        {
            if (
                JsExportAttributeHelper.HasAttribute(
                    member,
                    AttributeMetadataNames.JsIgnoreFromGlobalsAttribute
                )
            )
                continue;
            ...
            {
                var functionAttribute = JsExportAttributeHelper.GetAttribute(
                    method,
                    AttributeMetadataNames.JsGlobalFunctionAttribute
                );
                if (
                    (functionAttribute is null && jsMemberAttribute is null)
                    || !ShouldEmitMethod(method)
                )
                    continue;

                var name = JsExportAttributeHelper.GetMemberName(
                    method,
                    memberNaming,
                    functionAttribute,
                    jsMemberAttribute
                ...
                var isConstructor =
                    functionAttribute is not null
                    && JsExportAttributeHelper.GetNamedBool(functionAttribute, "IsConstructor");
                var length = functionAttribute is not null
                    ? JsExportAttributeHelper.GetNamedInt(functionAttribute, "Length")
                        ?? ComputeFunctionLength(method)
                    : ComputeFunctionLength(method);
                var parameters = method
                    .Parameters.Select(static p => new GlobalParameterModel(
                        p.Name,
                        p.Type,
                    ...
                    .ToArray();
                functions.Add(new(name, method, length, isConstructor, parameters));
                continue;
            }

            if (member is IPropertySymbol property && property.Parameters.Length == 0)
            {
                var propertyAttribute = JsExportAttributeHelper.GetAttribute(
                    property,
                ...
                properties.Add(
                    new(
                        ...
                        propertyAttribute is not null
                            ? JsExportAttributeHelper.GetNamedBool(
                                propertyAttribute,
                                "Enumerable",
                                true
                            )
    ...
    private static bool GetWritable(
        ...
        bool wasAnnotatedWithJsMember,
        bool canWrite
    )
    {
        if (
            propertyAttribute is not null
            && JsExportAttributeHelper.TryGetNamedBool(
                propertyAttribute,
                "Writable",
                out var writable
            )
