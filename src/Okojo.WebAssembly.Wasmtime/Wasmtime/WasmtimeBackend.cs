...
public sealed class WasmtimeBackend : IWasmBackend
{
    ...
    public IWasmInstance Instantiate(IWasmCompiledModule module, IWasmImportResolver imports)
    {
        ...
        if (module is not WasmtimeCompiledModule wasmtimeModule)
            throw new ArgumentException(
                "Module was not compiled by WasmtimeBackend.",
                nameof(module)
            );
        ...
        try
        {
            ...
            foreach (var import in wasmtimeModule.Imports)
            {
                if (!imports.TryResolveImport(import, out var wasmExtern))
                    throw new WasmLinkException(
                        $"Missing import '{import.ModuleName}.{import.Name}'."
                    );
                ...
                switch (wasmExtern)
                {
                    case WasmtimeFunctionWrapper function:
                        linker.Define(
                            import.ModuleName,
                            import.Name,
                            function.GetFunctionForStore(store)
                        );
                        break;
            ...
            return new WasmtimeInstanceWrapper(
                store,
                instance,
                CreateExports(store, instance, wasmtimeModule.Exports)
            );
        }
        catch (WasmtimeException ex)
        {
            throw new WasmLinkException(
                $"Failed to instantiate WebAssembly module: {ex.Message}",
                ex
            );
        }
    }
    ...
    {
        var store = new Store(new());
        return new WasmtimeMemoryWrapper(
            new(store, type.MinimumPages, type.MaximumPages, type.Is64Bit),
            type
        );
    }
    ...
    public IWasmTable CreateTable(WasmTableType type, object? initialValue = null)
    {
        ...
        var tableKind = type.ElementKind switch
        {
            ...
            _ => throw new NotSupportedException(
                $"Unsupported wasm table element kind: {type.ElementKind}"
            ),
        };
            ...
            switch (descriptor.Kind)
            {
                ...
                case WasmExternalKind.Memory:
                {
                    ...
                    if (memory is not null)
                        exports[descriptor.Name] = new WasmtimeMemoryWrapper(
                            memory,
                ...
                case WasmExternalKind.Table:
                {
                    ...
                    if (table is not null)
                        exports[descriptor.Name] = new WasmtimeTableWrapper(
                            table,
                            (WasmTableType)descriptor.Type
                        );
                    break;
                }
                case WasmExternalKind.Global:
                {
                    ...
                    if (global is not null)
                        exports[descriptor.Name] = new WasmtimeGlobalWrapper(
                            global,
