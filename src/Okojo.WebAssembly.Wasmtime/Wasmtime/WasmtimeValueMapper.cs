...
internal static class WasmtimeValueMapper
{
    ...
    public static ValueBox ToWasmtime(in WasmValue value)
    {
        return value.Kind switch
        {
            ...
            WasmValueKind.FuncRef => value.ObjectValue is null
                ? Function.Null
                : (Function)value.ObjectValue,
            WasmValueKind.ExternRef => value.ObjectValue is string s
                ? s
                : ValueBox.AsBox(value.ObjectValue),
    ...
    public static WasmValue ToOkojo(ValueBox value, WasmValueKind kind, Store? store = null)
    {
        return kind switch
        {
            ...
            WasmValueKind.FuncRef => new(
                WasmValueKind.FuncRef,
                store is null ? null : value.AsFunction(store)
            ),
