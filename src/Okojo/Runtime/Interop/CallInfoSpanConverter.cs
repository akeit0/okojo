...
public static class CallInfoSpanConverter
{
    ...
    public static void FillArgumentSpan(
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            destination[i] = value.IsNumber
                ? checked((byte)value.NumberValue)
                : info.GetArgument<byte>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((sbyte)value.NumberValue)
                : info.GetArgument<sbyte>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((short)value.NumberValue)
                : info.GetArgument<short>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((int)value.NumberValue)
                : info.GetArgument<int>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((uint)value.NumberValue)
                : info.GetArgument<uint>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((long)value.NumberValue)
                : info.GetArgument<long>(argumentIndex);
        }
    }
    ...
    )
    {
        for (var i = 0; i < destination.Length; i++)
        {
            ...
            var value = info.GetArgument(argumentIndex);
            destination[i] = value.IsNumber
                ? checked((ulong)value.NumberValue)
                : info.GetArgument<ulong>(argumentIndex);
        }
    }
    ...
    {
        for (var i = 0; i < destination.Length; i++)
            destination[i] = Convert.ToDecimal(
                info.GetArgumentDouble(startIndex + i),
                CultureInfo.InvariantCulture
            );
    }

    public static void FillArgumentSpan<T>(
        scoped in CallInfo info,
        int startIndex,
        Span<T> destination
    )
    {
        for (var i = 0; i < destination.Length; i++)
            destination[i] = HostValueConverter.ConvertFromJsValue<T>(
                info.Realm,
                info.GetArgument(startIndex + i)
            );
    }
}
