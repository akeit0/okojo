...
public partial class Intrinsics
{
        ...
        return new(
            ...
            (in info) =>
            {
                ...
                if (!realm.TryToObject(args[0], out var iterable))
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "Map iterable must not be null or undefined"
                    );
    ...
    private void InstallMapConstructorBuiltins()
    {
        ...
        var entriesFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                var realm = info.Realm;
                var thisValue = info.ThisValue;
                return new JsMapIteratorObject(
                    realm,
                    ThisMapValue(realm, thisValue),
        ...
        var getOrInsertFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var getOrInsertComputedFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var groupByFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                else if (items is JsStringObject stringObject)
                {
                    ...
                    {
                        AppendMapGroupByValueForCallback(
                            realm,
                            result,
                            callbackFn,
        ...
        [
            ...
            PropertyDefinition.Const(
                IdSymbolToStringTag,
                JsValue.FromString("Map Iterator"),
                configurable: true
            ),
        ];
        ...
        [
            ...
            PropertyDefinition.Mutable(
                ...
                JsValue.FromObject(getOrInsertComputedFn)
            ),
    ...
    private static void AddMapEntryFromIterableValue(
        JsRealm realm,
        JsMapObject map,
        JsFunction adder,
    ...
    )
    {
        if (!entryValue.TryGetObject(out var entry))
            throw new JsRuntimeException(
                JsErrorKind.TypeError,
                "Map iterable value must be an object"
            );

        var key = entry.TryGetElementWithReceiver(realm, entry, 0, out var keyValue)
            ? keyValue
            : JsValue.Undefined;
        var value = entry.TryGetElementWithReceiver(realm, entry, 1, out var valueValue)
            ? valueValue
            : JsValue.Undefined;
        var args = new InlineJsValueArray2 { Item0 = key, Item1 = value };
    ...
    )
    {
        ...
        if (result.TryGetValue(key, out var existingGroup))
        {
            if (
                ...
                throw new JsRuntimeException(
                    JsErrorKind.TypeError,
                    "Map.groupBy internal group must be an array"
                );
