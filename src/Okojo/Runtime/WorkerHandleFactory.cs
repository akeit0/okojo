...
internal static class WorkerHandleFactory
{
    ...
    )
    {
        ...
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.OnMessageError,
            JsValue.Undefined,
            JsShapePropertyFlags.Writable
        );

        var postMessageFn = new JsHostFunction(
            ownerRealm,
            static (in info) =>
            {
        ...
        var evalFn = new JsHostFunction(
            ...
            static (in info) =>
            {
                ...
                var data = (WorkerHandleRuntimeData)callee.UserData!;
                var source =
                    args.Length != 0 && args[0].IsString ? args[0].AsString() : string.Empty;
        ...
        var loadModuleFn = new JsHostFunction(
            ownerRealm,
            static (in info) =>
            {
                var realm = info.Realm;
        ...
        var terminateFn = new JsHostFunction(
            ownerRealm,
            static (in info) =>
            {
        ...
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.PostMessage,
            JsValue.FromObject(postMessageFn),
            JsShapePropertyFlags.Open
        );
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.Eval,
            JsValue.FromObject(evalFn),
            JsShapePropertyFlags.Open
        );
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.LoadModule,
            JsValue.FromObject(loadModuleFn),
            JsShapePropertyFlags.Open
        );
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.Pump,
            JsValue.FromObject(pumpFn),
            JsShapePropertyFlags.Open
        );
        handle.DefineDataPropertyAtom(
            ownerRealm,
            atoms.Terminate,
            JsValue.FromObject(terminateFn),
            JsShapePropertyFlags.Open
        );
