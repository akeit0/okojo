...
internal static class JsValueDebugString
{
    public static string FormatValue(in JsValue value)
    {
        ...
        if (value.IsNull)
            return "null";
        if (value.IsBool)
            return value.U == JsValue.True.U ? "true" : "false";
        if (value.IsInt32)
            return $"Number({value.Int32Value})";
        if (value.IsFloat64)
        {
            var v = value.Float64Value;
            if (Unsafe.BitCast<double, ulong>(v) == Unsafe.BitCast<double, ulong>(-0))
                return "Number(-0)";
            return $"Number({v.ToString(CultureInfo.InvariantCulture)})";
        }

        if (value.IsString)
            return $"String(\"{Escape(value.AsString())}\")";
        if (value.IsObject)
        {
