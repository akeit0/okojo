...
public partial class Intrinsics
{
    ...
    {
        var reflect = new JsPlainObject(Realm, false) { Prototype = ObjectPrototype };
        ...
        var constructFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    !args[0].TryGetObject(out var targetObj)
                    || targetObj is not JsFunction targetFn
                ...
                {
                    if (
                        !args[2].TryGetObject(out var newTargetObj)
        ...
        var definePropertyFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (args.Length < 2)
                    throw new JsRuntimeException(
                        JsErrorKind.TypeError,
                        "Reflect.defineProperty requires propertyKey"
                    );
                ...
                if (
                    args[0].TryGetObject(out var targetObj)
                    && args[2].TryGetObject(out var descriptorObj)
                )
                {
                    var canonicalDescriptor =
                        ProxyDescriptorUtilities.CreateCanonicalDescriptorObject(
                            realm,
                            descriptorObj
                        );
                    if (
                        targetObj.TryDefinePropertyFromDescriptorObject(
                            realm,
                            normalizedKey,
                            canonicalDescriptor,
                            out var proxyResult
                        ...
                        return proxyResult ? JsValue.True : JsValue.False;
                }
        ...
        var deletePropertyFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
                ...
                if (
                    ...
                    && TryDeleteTypedArrayIntegerIndexedElement(
                        realm,
                        typedArray,
                        normalizedKey,
                ...
                var isIndex = realm.TryResolvePropertyKey(
                    normalizedKey,
                    out var index,
                    out var atom
                );
                var ok = isIndex
                    ? target.DeleteElement(index)
                    : target.DeletePropertyAtom(realm, atom);
        ...
        var getFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                if (
                    target is JsTypedArrayObject typedArray
                    && TryGetTypedArrayIntegerIndexedElement(
                        realm,
                        typedArray,
                        normalizedKey,
                ...
                if (
                    receiverValue.TryGetObject(out var receiverObj)
                    && target.TryGetPropertyAtomWithReceiver(
                        realm,
                        receiverObj,
                        atom,
                        out var withReceiver,
                        out _
                    )
                )
                    return withReceiver;
        ...
        var getOwnPropertyDescriptorFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var descriptorArgs = new InlineJsValueArray2 { Item0 = args[0], Item1 = args[1] };
                return realm.InvokeObjectConstructorMethod(
                    "getOwnPropertyDescriptor",
                    descriptorArgs.AsSpan()
                );
            },
            "getOwnPropertyDescriptor",
            2
        ...
        var getPrototypeOfFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                return target.GetPrototypeOf(info.Realm) is { } prototype
                    ? prototype
                    : JsValue.Null;
            },
            "getPrototypeOf",
        ...
        var hasFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                var has = JsRealm.HasPropertySlowPath(
                    realm,
                    target,
        ...
        var ownKeysFn = new JsHostFunction(
            Realm,
            (in info) =>
            {
                var realm = info.Realm;
        ...
        var preventExtensionsFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                try
                {
                    ...
                    return target is IProxyObject || !target.IsExtensible
                        ? JsValue.True
                        : JsValue.False;
                }
                catch (JsRuntimeException ex) when (ex.Kind == JsErrorKind.TypeError)
            ...
            (in info) =>
            {
                ...
                if (isIndex)
                {
                    if (!receiverValue.TryGetObject(out var primitiveCheckedReceiver))
                        return ReflectSetWithPrimitiveReceiver(
                            realm,
                            target,
                            normalizedKey,
                            value,
                    ...
                    if (
                        !ReferenceEquals(target, primitiveCheckedReceiver)
                        && ReceiverBlocksDataPropertyWrite(
                            realm,
                            primitiveCheckedReceiver,
                            normalizedKey
                        )
                    )
                        return JsValue.False;
                    ...
                    {
                        if (
                            target is JsTypedArrayObject typedArrayTarget
                            && ReferenceEquals(target, receiverObj)
                        )
                            return typedArrayTarget.TrySetElement(index, value)
                                ? JsValue.True
                                : JsValue.False;
                ...
                if (
                    !ReferenceEquals(target, receiverObject)
                ...
                return target.TrySetPropertyAtom(realm, atom, value, out _)
                    ? JsValue.True
                    : JsValue.False;
            },
            "set",
        ...
        var setPrototypeOfFn = new JsHostFunction(
            ...
            (in info) =>
            {
                ...
                return ProxyCore.SetPrototypeOnTarget(realm, target, proto)
                    ? JsValue.True
                    : JsValue.False;
            },
            "setPrototypeOf",
    ...
    private static bool ReceiverBlocksDataPropertyWrite(
        JsRealm realm,
        JsObject receiver,
        in JsValue normalizedKey
    )
    ...
    )
    {
        ...
        for (var cursor = target; cursor is not null; cursor = cursor.Prototype)
        {
            if (isIndex)
            {
                ...
                return elementDescriptor.Writable
                    ? ReflectSetTargetKind.Data
                    : ReflectSetTargetKind.ReadOnlyData;
            }
            ...
            return namedDescriptor.Writable
                ? ReflectSetTargetKind.Data
                : ReflectSetTargetKind.ReadOnlyData;
        }
