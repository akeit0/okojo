...
internal static partial class ScratchRegExpMatcher
{
    ...
    public static RegExpMatchResult? Exec(CompiledProgram compiledProgram, string input, int startIndex)
    {
    ...
    private static RegExpMatchResult? TryExecWholeInputSimpleRun(ScratchRegExpProgram program,
        WholeInputSimpleRunPlan plan, string input, int begin)
    {
        if (begin != 0)
            return null;
    ...
    private static bool ConsumeCharacterSetRun(RegExpCharacterSet characterSet, string input, int pos,
    ...
    private static bool TryConsumeSpecialSimpleClassRun(RegExpSimpleClass simpleClass, string input, int pos,
        RegExpRuntimeFlags flags, int maxCount, out int endPos, out int consumed)
    {
        ...
        {
            case RegExpSimpleClassItemKind.Space:
                ConsumeWhitespaceRun(input, pos, maxCount, out endPos, out consumed);
                return true;
            case RegExpSimpleClassItemKind.NotSpace:
    ...
    private static RegExpMatchResult? ExecLinearBytecode(CompiledProgram compiledProgram, string input,
        int begin)
    {
        var program = compiledProgram.TreeProgram;
        var bytecodeProgram = compiledProgram.BytecodeProgram!;
        using var captureState = program.CaptureCount == 0 ? null : new RegExpCaptureState(program.CaptureCount);
        if (program.Flags.Sticky)
        {
            captureState?.Reset();
            return RegExpVm.TryMatch(compiledProgram, bytecodeProgram, input, begin, program.Flags,
                captureState,
                out var stickyEnd)
        ...
        {
            captureState?.Reset();
            if (RegExpVm.TryMatch(compiledProgram, bytecodeProgram, input, pos, program.Flags, captureState,
                    out var endIndex))
    ...
    private static RegExpMatchResult BuildBytecodeMatch(ScratchRegExpProgram program, string input, int startIndex,
        int endIndex, RegExpCaptureState? captureState)
    {
    ...
    private static bool TryMatchSimpleClassForward(RegExpSimpleClass cls, string input, int pos,
    ...
    private static bool TryMatchSimpleClassCodePoint(RegExpSimpleClass cls, int codePoint,
    ...
    private static bool SimpleClassItemMatchesCodePoint(RegExpSimpleClassItem item, int codePoint,
        RegExpRuntimeFlags flags)
    {
        return item.Kind switch
        {
            RegExpSimpleClassItemKind.Digit => codePoint is >= '0' and <= '9',
            RegExpSimpleClassItemKind.NotDigit => !(codePoint is >= '0' and <= '9'),
            RegExpSimpleClassItemKind.Space => IsSpace(codePoint),
            RegExpSimpleClassItemKind.NotSpace => !IsSpace(codePoint),
            RegExpSimpleClassItemKind.Word => IsWord(codePoint, flags.Unicode, flags.IgnoreCase),
            RegExpSimpleClassItemKind.NotWord => !IsWord(codePoint, flags.Unicode, flags.IgnoreCase),
            RegExpSimpleClassItemKind.Literal => CodePointEquals(codePoint, item.CodePoint, flags.IgnoreCase),
            RegExpSimpleClassItemKind.Range => CodePointInRange(codePoint, item.RangeStart, item.RangeEnd,
                flags.IgnoreCase),
            RegExpSimpleClassItemKind.PropertyEscape => FastPropertyEscapeMatches(item.PropertyEscape,
                codePoint, flags),
            _ => false
        };
    }
