...
internal sealed partial class JsPlannedFunctionCompiler
{
        ...
        switch (statement)
        {
            ...
            case JsReturnStatement returnStatement:
                if (returnStatement.Argument is not null)
                    EmitExpression(returnStatement.Argument);
                else
            ...
            case JsEmptyStatement:
                ...
                return;
            default:
                throw new NotSupportedException($"JsPlannedFunctionCompiler does not support statement '{statement.GetType().Name}'.");
        }
    }
    ...
    private void EmitVariableDeclaration(JsVariableDeclarationStatement declaration)
    {
        if (declaration.BindingPattern is not null)
            throw new NotSupportedException("Binding patterns are not supported by JsPlannedFunctionCompiler.");
