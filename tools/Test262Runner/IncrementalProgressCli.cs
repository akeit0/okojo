...
internal static class IncrementalProgressCli
{
    ...
    )
    {
        ...
        switch (NormalizeGroupBy(groupBy))
        {
            case "all":
                AppendTable(
                    ...
                    "By Category",
                    ApplyTop(
                        BuildRows(entries, static entry => GetCategoryKey(entry.RelativePath)),
                        top
                    )
                );
                AppendTable(
                    ...
                    "By Folder",
                    ApplyTop(
                        BuildRows(entries, static entry => GetFolderKey(entry.RelativePath)),
                        top
                    )
                );
        ...
        {
            if (
                string.Equals(normalizedListMode, "all", StringComparison.Ordinal)
        ...
        var skipReasons = entries
            .Where(static entry =>
                string.Equals(entry.Status, "skipped", StringComparison.Ordinal)
        ...
        var failureReasons = entries
            .Where(static entry =>
                string.Equals(entry.Status, "failed", StringComparison.Ordinal)
    ...
    private static string ResolveIncrementalJsonPath(string repoRoot, string? queryPathArg)
    {
        ...
        return Path.IsPathRooted(queryPathArg)
            ? queryPathArg
            : Path.GetFullPath(queryPathArg, repoRoot);
    }

    private static IncrementalProgressEntry NormalizeEntry(
        IncrementalProgressEntry entry,
        string resolvedRoot
    )
    {
    ...
    private static bool MatchesCategoryFilter(
        string relativePath,
        IReadOnlyCollection<string> categories
    )
    {
    ...
    private static bool MatchesFeatureFilter(
        IReadOnlyList<string> entryFeatures,
        IReadOnlyCollection<string> features
    )
    {
        if (features.Count == 0)
    ...
    private static bool MatchesUpdatedSince(
    ...
    )
    {
        return entries
            ...
            .Select(group =>
            {
                ...
                var passed = group.Count(static x =>
                    string.Equals(x.Status, "passed", StringComparison.Ordinal)
                );
                var failed = group.Count(static x =>
                    string.Equals(x.Status, "failed", StringComparison.Ordinal)
                );
                var skippedStandard = group.Count(static x =>
                    IsSkippedWithSpecStatus(x, SkipList.SkipSpecStatus.Standard)
                );
                var skippedLegacy = group.Count(static x =>
                    IsSkippedWithSpecStatus(x, SkipList.SkipSpecStatus.Legacy)
                );
                var skippedAnnexB = group.Count(static x =>
                    IsSkippedWithSpecStatus(x, SkipList.SkipSpecStatus.AnnexB)
                );
                var skippedProposal = group.Count(static x =>
                    IsSkippedWithSpecStatus(x, SkipList.SkipSpecStatus.Proposal)
                );
                var skippedFinishedProposalNotInBaseline = group.Count(static x =>
                    IsSkippedWithSpecStatus(
                        x,
                        SkipList.SkipSpecStatus.FinishedProposalNotInBaseline
                    )
                );
                var skippedOther = group.Count(static x =>
                    IsSkippedWithSpecStatus(x, SkipList.SkipSpecStatus.Other)
                );
                var skipped =
                    skippedStandard
                    + skippedLegacy
    ...
    private static IReadOnlyList<IncrementalRow> BuildFeatureRows(
        IReadOnlyList<IncrementalProgressEntry> entries
    )
    {
        return entries
            .SelectMany(entry =>
                entry.Features.Count == 0
                    ? [new("(none)", entry)]
                    : entry.Features.Select(feature => new KeyValuePair<
                        string,
                        IncrementalProgressEntry
                    >(feature, entry))
            )
            ...
            .Select(group =>
            {
                var total = group.Count();
                var passed = group.Count(static x =>
                    string.Equals(x.Value.Status, "passed", StringComparison.Ordinal)
                );
                var failed = group.Count(static x =>
                    string.Equals(x.Value.Status, "failed", StringComparison.Ordinal)
                );
                var skippedStandard = group.Count(static x =>
                    IsSkippedWithSpecStatus(x.Value, SkipList.SkipSpecStatus.Standard)
                );
                var skippedLegacy = group.Count(static x =>
                    IsSkippedWithSpecStatus(x.Value, SkipList.SkipSpecStatus.Legacy)
                );
                var skippedAnnexB = group.Count(static x =>
                    IsSkippedWithSpecStatus(x.Value, SkipList.SkipSpecStatus.AnnexB)
                );
                var skippedProposal = group.Count(static x =>
                    IsSkippedWithSpecStatus(x.Value, SkipList.SkipSpecStatus.Proposal)
                );
                var skippedFinishedProposalNotInBaseline = group.Count(static x =>
                    IsSkippedWithSpecStatus(
                        x.Value,
                ...
                var skippedOther = group.Count(static x =>
                    IsSkippedWithSpecStatus(x.Value, SkipList.SkipSpecStatus.Other)
                );
                var skipped =
                    skippedStandard
                    + skippedLegacy
    ...
    private static void AppendTable(
        TextWriter writer,
        string title,
        IReadOnlyList<IncrementalRow> rows
    )
    {
    ...
    private static IReadOnlyList<IncrementalRow> ApplyTop(
        IReadOnlyList<IncrementalRow> rows,
        int? top
    )
    {
    ...
    private static bool IsSkippedWithSpecStatus(
        IncrementalProgressEntry entry,
