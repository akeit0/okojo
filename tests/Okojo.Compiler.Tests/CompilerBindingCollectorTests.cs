...
public class CompilerBindingCollectorTests
{
    ...
    public void Collect_RootBindings_MatchesDirectLexicalSurface()
    {
        ...
        var rootBindings = result
            .Bindings.ToArray()
            .Where(binding => binding.ScopeId == result.RootScopeId)
            .ToArray();

        Assert.That(
            rootBindings.Select(static binding => (binding.Name, binding.Kind)).ToArray(),
            Is.EquivalentTo(
                new[]
                {
        ...
        Assert.That(
            rootBindings.Single(static binding => binding.Name == "alpha").Position,
            Is.GreaterThan(0)
        );
        Assert.That(
            rootBindings.Single(static binding => binding.Name == "Beta").Position,
            Is.GreaterThan(0)
        );
    }
    ...
    public void Collect_BlockAndLoopBindings_AssignsNestedScopes()
    {
        ...
        var loopBodyScopeId = bindings
            .Single(static binding => binding.Name == "bodyLocal")
            .ScopeId;

        Assert.That(
            scopes.Single(scope => scope.ScopeId == outerBlockScopeId).ParentScopeId,
            Is.EqualTo(0)
        );
        Assert.That(
            scopes.Single(scope => scope.ScopeId == loopScopeId).ParentScopeId,
            Is.EqualTo(0)
        );
        Assert.That(
            scopes.Single(scope => scope.ScopeId == loopBodyScopeId).ParentScopeId,
            Is.EqualTo(loopScopeId)
        );

        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == outerBlockScopeId)
        ...
        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == loopScopeId)
                .Select(static binding => (binding.Name, binding.Kind, binding.IsConst))
                .ToArray(),
            Is.EqualTo(new[] { ("i", CompilerCollectedBindingKind.LoopHeadAlias, false) })
        );

        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == loopBodyScopeId)
                .Select(static binding => (binding.Name, binding.Kind, binding.IsConst))
                .ToArray(),
            Is.EqualTo(new[] { ("bodyLocal", CompilerCollectedBindingKind.Lexical, false) })
        );
    }
    ...
    public void Collect_FunctionAndCatchBindings_RecordsParametersAndAliases()
    {
        ...
        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == functionScopeId)
                .Select(static binding => (binding.Name, binding.Kind))
                .ToArray(),
            Does.Contain(("first", CompilerCollectedBindingKind.Parameter))
        );
        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == functionScopeId)
                .Select(static binding => (binding.Name, binding.Kind))
                .ToArray(),
            Does.Contain(("second", CompilerCollectedBindingKind.Parameter))
        );
        Assert.That(
            bindings
                .Where(binding => binding.ScopeId == functionScopeId)
                .Select(static binding => (binding.Name, binding.Kind))
                .ToArray(),
            Does.Contain(("rest", CompilerCollectedBindingKind.Parameter))
        );
    ...
    public void Collect_RootLexicalBindings_RecordSourcePositions()
    {
        ...
        var rootBindings = result
            .Bindings.ToArray()
            .Where(binding =>
                binding.ScopeId == result.RootScopeId
        ...
        Assert.That(
            rootBindings.Select(static binding => (binding.Name, binding.Position)).ToArray(),
            Is.EqualTo(
                new[]
                {
