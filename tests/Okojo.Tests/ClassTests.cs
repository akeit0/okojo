...
public class ClassTests
{
    ...
    public void Class_Static_Private_Generator_Methods_Are_Stable_Across_Parallel_Engines()
    {
        ...
        Assert.That(
            failures,
            Is.Empty,
            failures.TryPeek(out var firstFailure) ? firstFailure : string.Empty
        );
    }
    ...
    public void ClassStaticBlock_Nested_Methods_Keep_Their_Own_Arguments_Binding()
    {
        ...
        for (var i = 0; i < 9; i++)
        {
            ...
            Assert.That(
                value.IsTrue,
                Is.True,
                $"static block nested method arguments check #{i} failed"
            );
        }
    }
    ...
    public void ClassPrivateField_SlotIndex_Above_ByteMaxValue_Works()
    {
        ...
        sourceBuilder
            .Append("  read() { return this.#f")
            .Append(privateFieldCount - 1)
            .AppendLine("; }");
        sourceBuilder
            .Append("  write(v) { this.#f")
            .Append(privateFieldCount - 1)
            .Append(" = v; return this.#f")
            .Append(privateFieldCount - 1)
            .AppendLine(" === v; }");
        ...
        var script = JsCompiler.Compile(
            realm,
            FlatJavaScriptParser.ParseScript(sourceBuilder.ToString())
        );
    ...
    public void ClassComputed_MethodObjectKey_ToString_SideEffectOrder_Works()
    {
        ...
        Assert.That(
            realm.Eval("Object.getOwnPropertyNames(C.prototype).join(',')").AsString(),
    ...
    public void Class_Computed_Accessors_Use_Runtime_Property_Key_For_Function_Name()
    {
        ...
        Assert.That(
            ...
            Is.EqualTo("get m|set m|get [test262]|set [test262]|get |set ")
        );
    }
