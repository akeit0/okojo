...
public class ParserIdentifierTableTests
{
    ...
    public void IdentifierTable_TryGetIdentifierId_ForSpan_Matches_Existing_Entry()
    {
        ...
        {
            Assert.That(
                table.TryGetIdentifierId("alpha".AsSpan(), out var lookedUpAlphaId),
                Is.True
            );
    ...
    public void ParseScript_ObjectAndClassPropertyParsing_Remains_Correct_With_Shared_PropertyKey_Helper()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                objectExpression.Properties[1].Kind,
                Is.EqualTo(JsObjectPropertyKind.Getter)
            );
            Assert.That(
                objectExpression.Properties[2].Kind,
                Is.EqualTo(JsObjectPropertyKind.Setter)
            );
            ...
            Assert.That(
                classDecl.ClassExpression.Elements[1].Kind,
                Is.EqualTo(JsClassElementKind.Getter)
            );
            Assert.That(
                classDecl.ClassExpression.Elements[2].Kind,
                Is.EqualTo(JsClassElementKind.Setter)
            );
