...
public class TypedArrayFamiliesFeatureTests
{
    ...
    {
        var realm = JsRuntime.Create().DefaultRealm;
        realm.Global["detachBuffer"] = JsValue.FromObject(
            new JsHostFunction(
                realm,
                (in info) =>
                {
                    ...
                    if (
                        ...
                        && obj is JsArrayBufferObject buffer
                    )
                        buffer.Detach();
    ...
    [Test]
    public void TypedArray_Of_Ignores_Write_That_Becomes_Out_Of_Bounds_During_Value_Coercion_And_Allows_Later_In_Bounds_Writes()
    {
        var realm = JsRuntime.Create().DefaultRealm;
        var script = JsCompiler.Compile(
            realm,
