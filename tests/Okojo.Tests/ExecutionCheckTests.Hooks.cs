...
public partial class ExecutionCheckTests
{
    ...
    public void PumpJobs_Emits_Boundary_Checkpoint()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint => checkpoint.Kind == ExecutionCheckpointKind.Pump),
            Is.True
        );
    }
    ...
    public void Call_And_Return_Hooks_Can_Be_Toggled_At_Runtime()
    {
        ...
        realm.Execute(script);
        Assert.That(
            debugger.Checkpoints.Any(checkpoint => checkpoint.Kind == ExecutionCheckpointKind.Call),
            Is.True
        );
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.Kind == ExecutionCheckpointKind.Return
            ),
            Is.True
        );
    }
    ...
    public void Call_And_Return_Hooks_Can_Be_Toggled_During_Debugger_Stop()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Count(checkpoint =>
                checkpoint.Kind == ExecutionCheckpointKind.Call
            ),
            Is.EqualTo(1)
        );
        Assert.That(
    ...
    public void CaughtException_Hook_Emits_On_First_Exception_Capture_Before_Catch_Routing()
    {
        ...
        var checkpoint = debugger.Checkpoints.Single(item =>
            item.Kind == ExecutionCheckpointKind.CaughtException
        );
