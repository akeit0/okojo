...
public class GeneratedDocDeclarationTests
{
    ...
    public async Task DocGenerator_Emits_Rest_Params_For_ReadOnlySpan()
    {
        ...
        try
        {
            ...
            Assert.That(
                globalsText,
                Does.Contain("declare function sumNumbers(...values: number[]): number;")
            );
            Assert.That(
                globalsText,
                Does.Contain("declare function describeAny(...values: any[]): string;")
            );
            Assert.That(
                globalsText,
                Does.Contain("declare function background(color: string): void;")
            );
            Assert.That(
                globalsText,
                Does.Contain("declare function background(gray: number): void;")
            );
            Assert.That(
                globalsText,
            ...
            Assert.That(
                objectText,
                Does.Contain("static sumNumbers(...values: number[]): number;")
            );
            Assert.That(
                objectText,
                Does.Contain("static describeJsValues(...values: any[]): string;")
            );
