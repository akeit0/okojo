...
public class OkojoDynamicPlainObjectTests
{
    ...
    public void JsonParse_Uses_DictionaryStart_PlainObject()
    {
        ...
        Assert.That(
            ((JsPlainObject)realm.Accumulator.AsObject()).UsesDynamicNamedProperties,
            Is.True
        );
        var parsed = (JsPlainObject)realm.Accumulator.AsObject();
        Assert.That(parsed.Slots.Length, Is.GreaterThanOrEqualTo(2));
        Assert.That(
            parsed.TryGetOwnNamedSlotIndex(realm.Atoms.InternNoCheck("a"), out var slotA),
            Is.True
        );
        Assert.That(slotA, Is.EqualTo(0));
        Assert.That(
            parsed.TryGetOwnNamedSlotIndex(realm.Atoms.InternNoCheck("b"), out var slotB),
            Is.True
        );
    ...
    public void DictionaryStartPlainObject_SetExistingProperty_Writes_Without_StaticSlotInfo()
    {
        ...
        Assert.That(
            obj.TrySetPropertyAtom(realm, atom, JsValue.FromInt32(7), out var slotInfo),
            Is.True
        );
