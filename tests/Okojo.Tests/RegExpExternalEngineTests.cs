...
public class RegExpExternalEngineTests
{
    ...
    public void RegExpCrossRealmGetterUsesGetterRealmTypeError()
    {
        ...
        var getterValue = otherRealm.Eval(
        ...
        var ex = Assert.Throws<JsRuntimeException>(() =>
            otherRealm.InvokeFunction(
                (JsFunction)getterObj!,
                realm.Eval("RegExp.prototype"),
    ...
    public void ExternalRegExpEngine_ScopedModifiersAffectEscapesAndPropertyClasses()
    {
        ...
        Assert.That(
            engine.Exec(engine.Compile(@"[\p{Hex}\P{Hex}]", "u"), "\uD834\uDF06", 0),
            Is.Not.Null
        );
    }
    ...
    public void ExternalRegExpEngine_SupportsCurrentUnicodePropertyFrontier()
    {
        ...
        Assert.That(
            engine.Exec(engine.Compile(@"\p{Case_Ignorable}+", "u"), "\u00AD", 0),
            Is.Not.Null
        );
        ...
        Assert.That(
            engine.Exec(engine.Compile(@"\p{Combining_Mark}+", "u"), "\u0301", 0),
            Is.Not.Null
        );
        Assert.That(
