...
public partial class ExecutionCheckTests
{
    ...
    public void PeriodicCheckpoint_ToPausedSnapshot_Mirrors_Checkpoint()
    {
        ...
        var checkpoint = debugger.Checkpoints.Find(checkpoint =>
        ...
        Assert.That(
            snapshot.GetDebuggerStopSummary(),
            Does.Contain(snapshot.CurrentFrameInfo.FunctionName)
        );
    }
    ...
    public void DebuggerAndBreakpoint_Checkpoints_Expose_Paused_Snapshots()
    {
        ...
        var debuggerStop = debugger.Checkpoints.First(checkpoint =>
            checkpoint.Kind == ExecutionCheckpointKind.DebuggerStatement
        );
        var breakpointStop = debugger.Checkpoints.First(checkpoint =>
        ...
        Assert.That(
            breakpointSnapshot.SourceLocation?.SourcePath,
            Is.EqualTo("paused-debugger.js")
        );
        Assert.That(
            breakpointSnapshot.StackFrames.Any(frame => frame.FunctionName == "add"),
            Is.True
        );
    }
    ...
    public void StepOver_Request_Emits_Step_Checkpoint()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint => checkpoint.Kind == ExecutionCheckpointKind.Step),
            Is.True
        );
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                ...
                && sourceLocation.SourcePath == "step-over.js"
            ),
            Is.True
        );
    }
