...
public partial class ExecutionCheckTests
{
    ...
    public void DebuggerStatement_Provides_StackTrace()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.GetDebuggerStopSummary().Contains("inner")
            ),
            Is.True
        );
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.StackFrames.Any(frame => frame.FunctionName == "inner")
            ),
            Is.True
        );
    }
    ...
    public void DebuggerStatement_Provides_Source_Position()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                ...
                && checkpoint.CurrentFrameInfo.SourceColumn > 0
            ),
            Is.True
        );
    }
    ...
    public void DebuggerStatement_Provides_Source_Path()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.GetDebuggerStopSummary().Contains("debug-stop.js")
            ),
            Is.True
        );
    }
    ...
    public void DebuggerStatement_Provides_Source_Path_And_Caller_Stack()
    {
        ...
        Assert.That(
            checkpoint.StackFrames.Any(frame => frame.SourcePath == "debug-stop-stack.js"),
            Is.True
        );
    ...
    public void Generator_Suspend_And_Resume_Emit_Boundary_Checkpoints()
    {
        ...
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.CurrentFrameInfo.FunctionName == "gen"
            ),
            Is.True
        );
        Assert.That(
            debugger.Checkpoints.Any(checkpoint =>
                checkpoint.GetDebuggerStopSummary().Contains("gen")
            ),
            Is.True
        );
        Assert.That(
