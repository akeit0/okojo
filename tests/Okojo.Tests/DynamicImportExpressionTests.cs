...
public class DynamicImportExpressionTests
{
    ...
    public void DynamicImport_WithTextType_ResolvesSyntheticTextModule()
    {
        ...
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/data.json"] = textSource }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/boom.js"] = """throw new TypeError("boom");""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/not-json.js"] = """export default 1;""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/value.json"] = """{"ok":true}""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
        ...
        for (
            var i = 0;
            i < 20 && realm.Global["done"].TryGetString(out var doneStr) && doneStr == "pending";
            i++
        )
            realm.Agent.PumpJobs();

        Assert.That(
            realm.Global["done"].TryGetString(out var finalDoneStr)
                && finalDoneStr.StartsWith("TypeError:"),
            Is.True
        );
    }
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/tests/module.js"] = """export default 1;""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/tests/module.js"] = """export const value = 1;""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/boom.js"] = """throw new URIError("bad");""" }
        );
        var realm = JsRuntime.CreateBuilder().UseModuleSourceLoader(loader).Build().MainRealm;
        var script = JsCompiler.Compile(
            realm,
