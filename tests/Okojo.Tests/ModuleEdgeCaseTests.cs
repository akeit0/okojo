...
public class ModuleEdgeCaseTests
{
    ...
    public void EvaluateModule_ExportDefaultAnonymousClass_HasNameDefault()
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal)
            {
                ["/mods/default-class.js"] =
                    """export default class { static value() { return 7; } }""",
            }
        );
    ...
    public void EvaluateModule_Static_And_Dynamic_Json_Imports_Share_The_Same_Default_Value()
    {
        ...
        for (
            var i = 0;
            i < 20 && realm.Global["done"].IsString && realm.Global["done"].AsString() == "pending";
            i++
        )
            realm.PumpJobs();
    ...
    public void EvaluateModule_ImportNamespace_ReflectsLiveExportValue()
    {
        ...
        var before = realm.InvokeFunction(
            (JsFunction)readObj!,
            JsValue.Undefined,
        ...
        var after = realm.InvokeFunction(
            (JsFunction)readObj!,
            JsValue.Undefined,
            ReadOnlySpan<JsValue>.Empty
        );
    ...
    public void EvaluateModule_ExportDefaultNamedDeclarationForms_Work()
    {
        ...
        var fnResult = realm.InvokeFunction(
            (JsFunction)defaultFnObj!,
            JsValue.Undefined,
            ReadOnlySpan<JsValue>.Empty
        );
    ...
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal) { ["/mods/main.js"] = """export const x = 1;""" }
        );
    ...
    public void EvaluateModule_ImporterOfAsyncCycleLeaf_Waits_For_CycleRoot_Completion()
    {
        ...
        Assert.That(
            Array.IndexOf(entries, "cycle leaf end"),
        ...
        Assert.That(
            Array.IndexOf(entries, "cycle root end"),
    ...
    public void EvaluateModule_ImporterOfThreeNodeAsyncCycle_Waits_For_Entire_Cycle()
    {
        ...
        Assert.That(
            Array.IndexOf(entries, "cycle a end"),
            Is.GreaterThan(Array.IndexOf(entries, "cycle a start"))
        );
        Assert.That(Array.IndexOf(entries, "cycle b start"), Is.GreaterThanOrEqualTo(0));
        Assert.That(
            Array.IndexOf(entries, "cycle b end"),
            Is.GreaterThan(Array.IndexOf(entries, "cycle b start"))
        );
        Assert.That(Array.IndexOf(entries, "cycle c start"), Is.GreaterThanOrEqualTo(0));
        Assert.That(
            Array.IndexOf(entries, "cycle c end"),
            Is.GreaterThan(Array.IndexOf(entries, "cycle c start"))
        );
        Assert.That(
            Array.IndexOf(entries, "cycle a end"),
    ...
    [Test]
    public void EvaluateModule_TopLevelAwait_RejectedPromise_Undefined_After_Prior_Rejection_Does_Not_Reuse_Previous_Value()
    {
        var loader = new InMemoryModuleLoader(
            new(StringComparer.Ordinal)
            {
