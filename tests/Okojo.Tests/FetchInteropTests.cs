...
public class FetchInteropTests
{
    ...
    public async Task FetchBuilder_Can_Use_Custom_HttpClient()
    {
        using var httpClient = new HttpClient(
            new StubHttpMessageHandler(static request =>
            {
                var method = request.Method.Method;
                var header = request.Headers.TryGetValues("x-test", out var values)
                    ? string.Join(",", values)
                    : "";
        ...
        Assert.That(
            result.AsString(),
            Is.EqualTo(
                "202|Accepted|text/plain; charset=utf-8|true|method=POST;header=ok;body=hello"
            )
        );
    }
    ...
    private static HttpResponseMessage AddHeader(
