...
public sealed partial class OkojoDebugServerIntegrationTests
{
    ...
    public async Task BreakpointUpdate_Publishes_Relocated_Line_Info()
    {
        ...
        var added = await server.WaitForJsonEventAsync(
            "breakpoint-added",
            TimeSpan.FromSeconds(10)
        );
        Assert.That(GetInt(added, "requestedLine"), Is.EqualTo(2));
        Assert.That(
            added.TryGetProperty("verified", out var addedVerified)
                && addedVerified.ValueKind == JsonValueKind.False,
            Is.True
        );
        ...
        var updated = await server.WaitForJsonEventAsync(
            "breakpoint-updated",
            TimeSpan.FromSeconds(10)
        );
        Assert.That(GetInt(updated, "requestedLine"), Is.EqualTo(2));
        Assert.That(
            updated.TryGetProperty("verified", out var updatedVerified)
                && updatedVerified.GetBoolean(),
            Is.True
        );
