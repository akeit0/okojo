...
public sealed partial class OkojoDebugServerIntegrationTests
{
    ...
    public async Task BytecodeCommand_Returns_Current_Disassembly_With_Stop_Marker()
    {
        ...
        Assert.That(
            GetString(bytecode, "title"),
            Does.Contain(Path.GetFileName(workspace.ScriptPath))
        );
        Assert.That(
            GetNestedString(bytecode, "sourceLocation", "sourcePath"),
            Is.EqualTo(workspace.ScriptPath)
        );
    ...
    public async Task BytecodeCommand_Resolves_Imported_Module_Stop()
    {
        ...
        await using var server = DebugServerProcess.Start(
            workspace.EntryPath,
            new[] { "--cwd", workspace.Root, "--module-entry", "--check-interval", "1" }
        );
